...
test('عضو بلا أي رحلة سابقة يملأ بروفايله، يُنشئ رحلته الخاصة، وبيانات بنكه تظهر فيها حيّة', async ({ page }) => {
  ...
  // ⚠️ تحقّق سلبي حقيقي (القاعدة ١٨): «نسخة احتياطية» للمسؤول العالمي وحده،
  // فغائب تماماً من الـ DOM. 🆕 أما «حذف الرحلة» فظاهر — منشئ الرحلة يحذفها
  // (manageTrip، بنفس شروط الأهلية)، والمنظّم المساعد لا (organizer-full-management).
