...
test('حذف مصروف ثم التراجع الفوري يعيده، وحذفه مجدداً ثم استعادته من سلة المهملات يعيده أيضاً', async ({ page }) => {
  ...
  // ⚠️ لا تأكيد على نصّ التوست هنا: السطر التالي يفحص الأثر الدائم (البطاقة
  // زالت)، والنقر على «تراجع» أدناه يثبت وجود التوست عملياً لا بالادّعاء —
  // فتأكيدٌ ثالثٌ عابر كان يضيف نقطة سقوط بلا تغطية إضافية.
