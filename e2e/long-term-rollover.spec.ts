...
test('إغلاق الشهر يُرحّل الأرصدة دون أن يغيّر صافي رصيد أي عضو', async ({ page }) => {
  ...
  // فتح النافذة ثم الضغط على الإغلاق — 🆕 الضغط يُبدّل محتواها إلى التأكيد
  // داخل نفس النافذة (انظر LongTermModal.tsx)، لا نافذة تأكيد تحلّ محلّها.
...
test('ملف المسافر في رحلة طويلة يفتح نافذة الخروج لا تأكيد الحذف المعتاد', async ({ page }) => {
  ...
  // ⚠️ الحارس: تأكيد الخروج يظهر، **ولا** يسلك المسار حذف الرحلة القياسية.
  // 🆕 كان الدليل غياب «نعم، احذف» — وقد حُذفت نافذة التأكيد تلك من التطبيق
  // كلّه، فصار ذلك التأكيد يمرّ دوماً ولا يميّز شيئاً. الدليل الآن غياب تنبيه
  // الحذف الليّن: لو أخذ المسار فرع الرحلة القياسية لظهر «تم نقل المسافر إلى
  // سلة المهملات» فوراً بدل أن يظهر تأكيد تسوية ينتظر قراراً.
  // 🆕 التأكيد قسمٌ داخل ملف المسافر يحلّ محلّ الزرّ — لا نافذة مستقلّة يُغلق
  // الملف ليفسح لها كما كان (انظر TravelerProfileModal.tsx). فالملف باقٍ،
  // وزرّ «إخراج» الدقيق وحيد لأن «إخراج من الرحلة» اختفى حين حلّ التأكيد محلّه.
  await expect(page.getByRole('heading', { name: MONA.name, exact: true })).toBeVisible()
  await expect(page.getByText('تم نقل المسافر إلى سلة المهملات')).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'إخراج من الرحلة', exact: true })).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'إخراج', exact: true })).toBeVisible()
})
...
test('خروج منتدَب: يُمنع برصيد غير مسوّى، ويمرّ عبر «تسوية وخروج»', async ({ page }) => {
  ...
  // زرّ التأكيد يحمل نصّاً أقصر («تسوية وخروج» بلا «من الرحلة»). 🆕 التأكيد
  // يحلّ محلّ الزرّ الأول داخل الملف نفسه، فلا يجتمعان — لكن `exact` يبقى
  // صريحاً: مطابقة جزئية كانت ستلتقط الاثنين لو اجتمعا يوماً.
  await page.getByRole('button', { name: 'تسوية وخروج', exact: true }).click()
