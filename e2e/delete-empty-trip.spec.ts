...
test('مسؤول يحذف مصروفاً ونفسه من مسافري رحلة أنشأها، ثم يستطيع حذف الرحلة الفارغة فعلياً', async ({ page }) => {
  ...
  // لا من «رحلاتي» — انظر docs/DECISIONS.md. قسم الحذف مدموج في تبويب
  // «إعدادات الرحلة» المفتوح افتراضياً — لا تبويب مستقل يُضغط عليه. ──────────
  ...
  // ⚠️ الأثر الدائم لا الإشعار العابر. التوست يعيش 5 ثوانٍ ثم يزول، فتأكيدٌ
  // عليه ميزانيته 5s بينما لبقية الخطوات 45s — تفاوتٌ هو الهشاشة بعينها.
  // والزر يُصيَّر داخل map المسافرين (TravelerSection.tsx)، فغيابه يعني أن آخر
  // مسافر نشِط زال فعلاً — وهو بعينه الشرط الذي تحتاجه إعادة المحاولة أدناه.
  await expect(page.getByRole('button', { name: 'حذف المسافر' })).toHaveCount(0)
