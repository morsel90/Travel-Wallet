...
// لمحتواها — motion يعترض أي سحب رأسي في أي مكان من اللوحة (لا فقط
...
test('يمكن تمرير محتوى النافذة عند تجاوزه الشاشة — سحب المحتوى لا يُفسَّر كسحب لإغلاق النافذة', async ({ page }) => {
  ...
  // ⚠️ 🆕 إعادة التمرير إلى الأعلى صراحةً قبل القياس. النقر على زرّ يقع خارج
  // الشاشة يجعل المتصفح يُمرّر إليه تلقائياً (scroll-into-view)، والنموذج صار
  // أطول بعد أن خرج «من دفع؟» إلى قسم ظاهر — فكان القياس يبدأ من موضع غير
  // صفري (148px) ويسقط الاختبار على شرط ابتدائي لا على ما يقصده. وهذا الشرط
  // ليس زينة: بدونه يمرّ `scrollTopAfter > 0` مجاناً حتى لو لم يُمرِّر السحبُ
  // شيئاً إطلاقاً — أي أن الاختبار يصير أخضر بلا أن يفحص الانحدار الذي كُتب له.
