...
**محاولة أولى مرفوضة: زرّ «تعديل» على كل بطاقة في «رحلاتي».** كانت الخطوة الطبيعية الأولى — نقل `TripDetailPanel` إلى داخل القائمة، وزرّ يفتحها لمن يملك الصلاحية. عملت فعلاً، لكن صاحب الحساب فضّل شيئاً أبسط: **الضغط على اسم الرحلة في الهيدر** يفتح تعديلها مباشرة (`EditTripModal.tsx`، مودال كامل الشاشة يستدعي `TripDetailPanel` كما هي بلا تغيير) — بلا المرور بـ«رحلاتي» أصلاً لتعديل الرحلة *المفتوحة حالياً*. «رحلاتي» رجعت تنقّلاً بحتاً (فتح/إنشاء/استعادة)، ولا تعديل منها إطلاقاً.

**الأثر العملي الوحيد:** تعديل رحلة *أخرى* غير المفتوحة يحتاج خطوة إضافية — فتحها أولاً من «رحلاتي» (المسؤول يرى كل الرحلات هناك ويدخل أيّاً منها بصلاحية `isAdmin()`، بصرف النظر عن عضويته) ثم الضغط على اسمها بعد أن تصبح هي المفتوحة. لا قدرة فُقدت، خطوة واحدة زادت — تبادل قبله صاحب الحساب صراحة إذ كان يعرف أصلاً أن المسؤول يدخل أي رحلة بلا قيد.
