...
**ما يفعله `recordSettlement` بدلاً من ذلك:** يُطبّق التحويل على الطرف الدائن الوحيد في الدفتر (`deposited`) — المَدين دفع نقداً فصار كأنه أودع أكثر، والدائن استلم فصار كأنه أودع أقل — ويكتب سطرَي تدقيق (`add` للمَدين و`subtract` للدائن) في نفس المعاملة. **مجموع المُودَع لا يتغيّر**: تحويل بين طرفَين لا إيداع جديد. والتسوية تختفي من القائمة لأن الرصيدَين صارا صحيحَين، لا لأن أحداً أشّر عليها — وهذا وحده ما يجعلها تبقى مختفية بعد إضافة مصاريف جديدة.
