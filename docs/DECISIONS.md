...
### 🆕 التعقيد الداخلي مقبول، التعقيد الظاهر لا — لغة الواجهة مفصولة عن الـstate

للتطبيق ثماني حالات داخلية حول الرحلة والشهر: رحلة مفتوحة/مغلقة، رحلة طويلة المدى، دورة حالية/مغلقة، ترحيل، رصيد تراكمي، رصيد الدورة. **هذا ممتاز داخلياً ولا يُمسّ** — الفروق حقيقية ومفروضة في `firestore.rules` و`closeMonth`، وطمسها هناك خطأ أمني ومحاسبي. القرار أن المستخدم لا ينبغي أن يرى أياً منها.

المبدأ الحاكم: **الواجهة تصف الأثر، لا تسمّي الحالة.** المستخدم لا يسأل «في أي state أنا؟» بل «ماذا أستطيع الآن؟» و«كم لديّ؟». فحيثما كان الاسم الداخلي مسرَّباً، حلّ محلّه وصفُ ما يترتّب عليه:

- **«دورة»/«فترة» → اسم الشهر أو «هذا الشهر».** التطبيق يعرف «أغسطس 2026» أصلاً، فالكلمتان كانتا غلافاً فوق اسم مفهوم بلا شرح. والتقسيم بين السطحين مقصود ولا يُوحَّد: الشاشة الرئيسية تقول **«هذا الشهر»** (بديهي وهي مفتوحة الآن)، والتقارير والطباعة وExcel تقول **«أغسطس 2026»** (تُقرأ بعد شهور وخارج التطبيق، حيث «الحالية» بلا معنى).
- **«ميزانية الدورة»/«متبقي الدورة» → «المودع»/«المتبقي»**، نفس كلمتَي الرحلة القياسية. ما يختلف هو مصدر الرقم (محفظة الشهر لا الإجمالي التراكمي) لا معناه؛ واختلافُ الحساب تعقيدٌ داخلي، وإظهاره في التسمية يحوّله إلى تعقيد ظاهر ويجعل المستخدم يتعلّم مصطلحاً لمجرّد أن رحلته طويلة.
- **«ترحيل» → الأثر أو الرقم.** الزرّ يقول «إغلاق أغسطس 2026 وبدء سبتمبر 2026» لا «وترحيل الأرصدة»، وفي مودال التأكيد **حلّ الرقم بإشارته محلّ الجملة**: `+800.00` بدل «يُرحَّل له 800.00 ريال». الاتجاه محفوظ كاملاً في الإشارة واللون، بلا إلزام أحدٍ بمعرفة معنى «الترحيل» ليقرأ رقماً. وهذا هو نمط «الرقم أولاً، والتفصيل عند الطلب»: كشف الحساب التفصيلي — سطحُ الطلب — هو وحده الذي يقول «من الشهر السابق».
- **`completed`/`archived` → «انتهت الرحلة — تسجيل المصاريف متوقّف» و«رحلة قديمة — للاطّلاع فقط».** الحالتان تبقيان مفصولتين في الكود والقواعد (الأولى تقبل تسوية الأرصدة والثانية لا تقبل شيئاً)، لكن الاسم لا يصل الشاشة. و«مؤرشفة» تحديداً كانت تضلّل: المألوف عنها (أرشيف واتساب) أنها **مكان تخزين** يُخرَج منه متى شئت، لا أن العمليات المالية أُقفلت نهائياً.

⚠️ **الضمانة تأكيدات نفي لا إثبات فقط.** `tripStatus.test.ts` تؤكّد أن الرسالة **لا تحوي** «منتهية»/«مؤرشفة»، و`Header.test.tsx` تؤكّد غياب «دورة» واسم الشهر عن الشاشة الرئيسية. بلا هذه، تعود المصطلحات بأول تحرير نصّي بلا أن يسقط اختبار واحد — وهي عودة صامتة بطبعها، إذ لا شيء «يتعطّل» حين تتسرّب كلمة.

⚠️ **وهذا لا يشمل لوحة الإدارة.** أزرار «حالة الرحلة» الثلاثة هناك تسمّي الحالات صراحةً عن قصد: سطحُ *تحكّم* يختار المسؤول منه حالةً بعينها، ولا يمكن اختيار ما لا يُسمّى. الفصل بين «سطح القراءة» و«سطح التحكّم» هو حدّ هذا القرار.

### 🆕 تسجيل المصروف: خمس خطوات ظاهرة، وكل ما عداها يظهر عند الحاجة وحدها

المسار المعتمَد للنموذج الكامل، بهذا الترتيب: **المبلغ ← ماذا كان؟ ← من دفع؟ ← المشاركون ← حفظ**. وهذه العملية الأكثر تكراراً في التطبيق، فكل حقل يقف في طريقها يُدفع ثمنه عشرات المرات في الرحلة الواحدة.

**«من دفع؟» ليس تفصيلاً متقدّماً — وهو الخطأ الذي كان قائماً.** كان محدِّده مدموجاً مع التاريخ خلف سطر رمادي واحد (`اليوم • الصندوق المشترك`)، وهيئته البصرية هيئة نصّ لا زرّ. والنتيجة أن الحقل الذي يحدّد **كل** الأرصدة والتسويات كان يُحسم بالافتراضي صمتاً في الغالب. صار كبسولات ظاهرة دائماً: القيمة الحالية مرئية بلا نقرة، والتغيير نقرة واحدة.

⚠️ **الافتراضي بقي «الصندوق المشترك» عمداً، ولا يُغيَّر إلى «أنا».** الترتيب وحده هو ما يراعي المستخدم: ملفّه يتصدّر الكبسولات بعلامة «(أنا)» حين يكون حسابه مربوطاً بمسافر. أما جعله الافتراضي فيقلب محاسبة كل رحلة تعمل بصندوق مشترك حقيقي (الإيداعات المسبقة) — يُنسَب كل مصروف إلى دافع شخصي فتتضخّم مديونيات وهمية بلا أن يلحظ أحد. الترتيب اقتراح، والافتراضي حكم.

**وظهورُ الجميع ليس شرطاً في «ظاهر».** في رحلة من أربعة عشر مسافراً تملأ الكبسولات أربعة صفوف فيبتلع القسمُ نصفَ الشاشة ويدفع «المشاركون» والتاريخ خارجها — أي أن علاج الدفن أنتج دفناً من الجهة المقابلة. الظاهر ثلاثة: ملفّي، ثم الأحدث دفعاً فعلياً في هذه الرحلة (مشتقّ من `paidBy` في المصاريف الموجودة أصلاً — لا إعداد ولا سؤال إضافي)، والبقية خلف كبسولة «غيرهم (n)». والحدّ مشروط: خمسة مسافرين أو أقلّ يظهرون كلهم، إذ لا فائدة من إخفاء اثنين.

⚠️ **والدافع المختار مستثنى من الطيّ دائماً.** عند تعديل مصروف قديم دفعه مَن لم يدفع منذها، يقع خارج الثلاثة — وإخفاؤه يجعل القسم يعرض قيمةً غير قيمته الفعليّة. القاعدة العامة: **لا يُطوى ما يحمل القيمة الحالية، إنما تُطوى البدائل.**

**والخيارات المتقدّمة تظهر عند الحاجة لا قبلها:**
- **العملة**: سطر باهت ما دام المصروف بالريال، ومحدِّد كامل بنقرة — أو من البداية حين تكون العملة أجنبية أصلاً (تعديل مصروف سابق)، فسطر «عملة أخرى؟» فوق مصروف بالدولار كذب.
- **التقسيم غير المتساوي**: كما كان، خلف سطر الملخّص.
- **التاريخ**: سطر هادئ وحده الآن.

**والفئة تُشتقّ ولا تُسأل** (`utils/categoryGuess.ts`). من كتب «عشاء» قال الفئة أصلاً؛ وسؤاله عنها بعد ذلك سؤالٌ عمّا أجاب عنه للتوّ. ثلاثة قيود تجعل الاشتقاق اقتراحاً لا حكماً: لا يظهر قبل الوصف، ويُعرَض ككبسولة قابلة للتغيير بنقرة، وأول تغيير يدوي يوقفه لبقية عمر النموذج (وعند **تعديل** مصروف قائم يبدأ موقوفاً — فئته المحفوظة قرار سابق لا فراغ يُملأ بالتخمين).

⚠️ **ومفردات الاشتقاق مقصورة على الواضح تماماً.** كل كلمة محتملة لفئتين — «تذكرة» (طيران أم متحف؟)، «رحلة» (مواصلات أم نشاط؟) — مستبعَدة عمداً: تخمين خاطئ يصنّف المصروف في غير موضعه **بصمت**، بينما غياب التخمين يُبقيه «أخرى» — وهو ما كان يحدث لكل مصروف أصلاً. لهذا السبب نفسه يُطبَّق الاشتقاق على المسار السريع (`handleQuickAddExpense`) الذي كان يكتب «أخرى» دائماً فيُفرغ رسم توزيع الفئات من معناه، **دون** أن يُضاف إليه سؤال: الشريط السريع يبقى حقلين، وهذا حدّه.

### 🆕 الشاشة الرئيسية ثلاثة أقسام فقط — وكل ما عداها خلف زرّ «المزيد»

الشاشة كانت تعرض سبعة أقسام في تدفّق واحد (المقطع القادم، أرصدة المسافرين، الإحصائيات، الشهر المحاسبي، نموذج المصروف، الحساب البنكي، سجلّ المصاريف) وأزرارَ تقارير/سلة مبعثرة داخلها. صارت: **المصاريف ← الأرصدة ← المسافرون**، بهذا الترتيب تحديداً — ما يُفعل يومياً، ثم ما يُسأل عنه عند التصفية، ثم من يخصّهم. طلب صاحب الحساب صراحةً تقليل الحمل البصري.

**«الأرصدة» تعني التسويات (من يدفع لمن) لا بطاقات المسافرين.** الاسمان متقاربان («أرصدة المسافرين» عنوان قسم ثالث قائم)، والفرق جوهري: التسويات تجيب عن «كم أدين ولمن أُحوِّل؟» — الفعل الأكثر تكراراً في رحلة جماعية — وكانت مدفونة كتبويب أول داخل بطاقة «ملخص وإحصائيات الرحلة» أسفل الصفحة، بجانب رسمَين يُنظر إليهما مرة أو مرتين في الرحلة كلها. استُخرجت إلى `SettlementsPanel.tsx` كقسم رئيسي، وبقي `ChartsSection` بتبويبَيه الآخرين وانتقل كاملاً خلف القائمة.

**وزرّ فتحها هو اسم الرحلة/الشعار في الهيدر، لا زرّ `⋯` منفصل.** كانت الصيغة الأولى زرّاً مستقلاً بجانب «حسابي» — وهو حرفياً العنصر الذي رُفض من قبل حين اقتُرح كأيقونة قلم: «عنصر إضافي يزحم هيدراً مزدحماً أصلاً» (انظر القرار التالي في هذا الملف). ولاحظ صاحب الحساب أن القائمة تحوي الإعدادات وإدارة الرحلة أيضاً، فمكانها الطبيعي خلف هوية الرحلة نفسها — نمط اسم المجموعة في واتساب واسم القناة في Slack: الضغط على العنوان يفتح «كل ما يخصّ هذا الشيء». وأُضيف سهم `▾` بجانب الاسم، إذ بلا شيء يوحي بأن العنوان قائمة تصبح صحيحة وغير قابلة للاكتشاف في آنٍ واحد.

**ومكسب لم يكن مقصوداً: اسم الرحلة كان عنصراً ميّتاً لأغلب الأعضاء.** كان قابلاً للضغط لمن يملك تعديل الرحلة وحده (`canEditTrip`) — و`organizer-role.spec.ts` كان يؤكّد أن العضو العادي لا يراه زرّاً أصلاً. صار الآن مفيداً للجميع: التقارير والإحصائيات والمسار حقٌّ لكل عضو، و«إدارة الرحلة» تبقى بنداً مشروطاً *داخل* الورقة. أي أن الحارس انتقل من **وجود الزرّ** إلى **بندٍ داخله** — وهو الموضع الصحيح له (القاعدة ١٧).

⚠️ **الثمن المقبول: «إدارة الرحلة» صارت بنقرتين بدل واحدة.** وهذا يعكس تفضيلاً سابقاً صريحاً (رُفض «زرّ تعديل داخل رحلاتي» لصالح تعديل الرحلة المفتوحة مباشرة من الهيدر)، فلم يُتَّخذ إلا بعد مراجعته مع صاحب الحساب واختياره صراحةً — نقطة دخول واحدة لا مسارَين. والتعديل فعل نادر (اسم، مسار، أعضاء، دعوات) بينما التقارير والأرصدة يومية، فالمسار الأقصر أُعطي للأكثر تكراراً.

⚠️ **وأثره على ميزانية الاختبارات حقيقي**: كل مسار إداري في E2E صار يمرّ بنافذة إضافية بحركة زنبركية قبل وجهته، و`openTripDetailFromHeader` وحدها تُستدعى في خمسة ملفات. رُفعت `timeout` في `playwright.config.ts` من 30s إلى 45s — الميزانية القديمة وُضعت حين كانت أفعال الهيدر ضغطةً واحدة.

**ورقة سفلية لا قائمة منسدلة كـ`AccountMenu`.** البنود تصل إلى سبعة ونصفها مشروط بالصلاحية أو بنوع الرحلة، فقائمة منسدلة ضيّقة بارتفاع متغيّر كانت ستتجاوز حافة الشاشة على الجوال. و`Modal` يوفّر حصر التركيز وEscape والسحب-للإغلاق بلا أي كود إضافي.

**ولا تُكرَّر أي نقطة دخول.** لا زرّ «تعديل الرحلة» منفصل بعد الآن — صار بنداً في الورقة كغيره؛ والتقارير/الإحصائيات/المسار/السلة لا وجود لها في أي مكان آخر — نفس المبدأ الذي أزال «تصدير Excel» و«إدارة الرحلات» من مواضعهما المكرَّرة سابقاً. واختيارية كل بند في `MoreMenuActions` هي حارس صلاحيته: غياب الخاصية هو التعطيل، لا شرطٌ داخل `MoreMenuSheet`.

⚠️ **«سلة المهملات» انتقلت من ذيل سجلّ المصاريف إلى القائمة — وهذا لا ينقض السبب الذي وضعها هناك.** كان الشرط أن تبقى ظاهرة حين يحذف المسؤول *آخر* مصروف (فتحلّ شاشة «لا توجد مصاريف بعد» محلّ القائمة، وتختفي معها طريق التراجع في اللحظة التي وقع فيها الخطأ). موضعها الجديد في الهيدر لا يعتمد على حالة القائمة إطلاقاً — أي أنه يُرى في *كل* الحالات لا في حالة القائمة وحدها. الضمانة انتقلت مع البند إلى `MoreMenu.test.tsx`.

⚠️ **وسجلّ المصاريف في رأس الصفحة استلزم تمريراً صريحاً بعد كل تسجيل.** شريط الإدخال السريع ثابت أسفل الشاشة، أي أن المستخدم غالباً ممرَّر لأسفل حين يسجّل مصروفاً — فيُضاف المصروف في رأس قائمة خارج نطاق رؤيته تماماً، و`react-virtuoso` لا تكتفي بإخفاء الصفّ بل لا تُركّبه أصلاً. التنبيه (Toast) يقول «تم» لكنه لا يُري النتيجة. **جُرِّبت ثلاث آليات وقيست تحت حمل متوازٍ:** `Virtuoso.scrollToIndex` لا يتحرّك أحياناً (يقارن بحالته الداخلية عن موضع القائمة، وهي قديمة لحظة إضافة صفّ في الرأس)، و`scrollIntoView` يتحرّك ثم **يرتدّ** (المكتبة تعوّض إزاحة المحتوى بتحريك تمرير النافذة بنفسها بعد جزء من الثانية). المعتمَد: إحداثية محسوبة على النافذة، ونداء ثانٍ بعد استقرار التعويض — إن لم يرتدّ فهو تمرير إلى المكان نفسه بلا أثر مرئي.

⚠️ **ومعه `focus({ preventScroll: true })` في `useDialogA11y.ts`.** إعادة التركيز عند إغلاق النافذة واجبٌ وصولي (يعود المستخدم إلى حيث كان في ترتيب التنقّل)، أما تحريك الصفحة فليس جزءاً منه — والمتصفح يفعله افتراضياً. زرّ فتح نموذج المصروف يقع في الشريط الثابت أسفل الشاشة، فإعادة التركيز إليه بعد الحفظ كانت تجرّ الصفحة للأسفل وتُلغي التمرير إلى السجلّ.

### 🆕 المسار على الشاشة مكان واحد — أما تقرير الطباعة فيحتفظ بجدوله

بعد أن استقلّ المسار بصفحته خلف «المزيد»، صارت قائمة المقاطع تُعرض في موضعين: تبويب «تفصيل كامل الرحلة» في التقارير، ونافذة «مسار الرحلة». أُزيل الأول — نفس مبدأ «لا تُكرَّر أي نقطة دخول» الذي أزال «تصدير Excel» و«إدارة الرحلات» من مواضعهما المكرَّرة.

⚠️ **والطباعة استثناء مقصود لا سهو.** `PrintableTripReport` ما زال يستقبل `itinerary` ويرسم جدول المسار: **التقرير المصدَّر يُقرأ خارج التطبيق** — يُطبع أو يُشارَك كـPDF — حيث لا وجود لنافذة المسار ولا لأي نقرة تفتحها، فجدول المقاطع هناك مرجع المسافر لا تكرار. الحجّة التي أزالت القائمة من الشاشة (وجود مكان آخر أقرب) لا تنطبق على ورقة تُقرأ بلا تطبيق.

**ولهذا الضمانة ثنائية**: `ReportsView.test.tsx` يؤكّد الغياب عن `<main>` **والحضور** داخل `#print-root` معاً. اختبار غياب وحده كان سيبقى أخضر لو حُذف المسار من الطباعة أيضاً — وهو الانهيار الوحيد الذي يهمّ هنا (القاعدة ١٨). و`#print-root` قابل للاختبار لأنه في DOM دائماً ويُخفى بـ`@media screen` لا بشرط في React.

**وسقطت مع الإزالة خاصية `bare`.** كانت تخدم مستدعيَين: بطاقة بيضاء بعنوان «مسار الرحلة» للتقارير، وقائمة عارية للنافذة التي توفّر الاثنين أصلاً. بذهاب الأول صار العَلَم صحيحاً دائماً وفرعُ العنوان غير قابل للبلوغ، فحُذف. لا تُعِد الغلاف لمستدعٍ جديد قبل التأكد أنه لا يوفّره بنفسه.

### 🆕 شريط المقطع القادم — الاستثناء الوحيد لقاعدة «ثلاثة أقسام»، وهو سطر لا قسم

نقل المسار كاملاً خلف «المزيد» صحّح حملاً بصرياً، لكنه دفن سؤالاً يُسأل يومياً في رحلة قائمة: **«كم بقي، وإلى أين؟»** — سؤالٌ جوابه نظرة لا تنقّل، فصار خلف نقرتين. أعاده `NextSegmentStrip` (في `NextSegmentWidget.tsx`) إلى الشاشة الرئيسية فوق «المصاريف» مباشرة.

**سطر واحد لا قسم، وهذا هو كل الفرق.** طلب صاحب الحساب صراحةً: «أحتاجه في الرئيسية لغرض واحد فقط — معرفة الأيام المتبقية والوجهة سريعاً بمجرد النظر، دون أن يزاحم المصاريف». فالشريط يحمل ثلاثة عناصر ولا رابع: أيقونة وسيلة التنقّل، اسم الوجهة، والعدّ التنازلي (`✈️ الدوحة • بعد 3 أيام`). **ولا يحمل عمداً**: وقت الانطلاق، ولا موضعه، ولا رقم الرحلة، ولا PNR، ولا المقطع الثاني — كلها في صفحة المسار التي يفتحها الضغط عليه. الغيابات هنا ليست نقصاً بل هي العقد، ويحرسها `NextSegmentWidget.test.tsx` بتأكيدات نفي صريحة: أي تفصيل يتسرّب إلى الشريط يعيده إلى منافسة المصاريف، وهو بالضبط ما وُضع لتجنّبه.

⚠️ **الشريط والبطاقة ليسا مقاسَين لشيء واحد بل دوران مختلفان.** `NextSegmentWidget` (البطاقة) تعيش داخل صفحة المسار حيث جاء المستخدم ليقرأ المسار، فتعرض الوقت والتاريخ. `NextSegmentStrip` يعيش وسط المصاريف حيث جاء المستخدم لشيء آخر تماماً. لذلك لم تُعمَّم البطاقة بخاصية `compact` — الاختلاف في *ما يُعرض* لا في *كم يُعرض*، وخاصيةٌ كهذه كانت ستدعو لاحقاً إلى تسريب حقل «صغير» تلو الآخر. المشترك بينهما (`useMinuteTick`، `TRANSPORT_ICON`، `findNextSegment`، `formatCountdown`) يعيش مرة واحدة في الملف نفسه.

**والغياب تام لا شريط فارغ**: بلا مسار، أو حين تصير كل المقاطع في الماضي، يُرجع `null` فتعود الشاشة إلى أقسامها الثلاثة تماماً — لا سطر يشغل مكاناً ليقول «لا شيء». ومع نبضة النصف دقيقة يسقط المقطع من الشريط من تلقائه بمجرد أن يمرّ وقت انطلاقه.
### 🆕 حارس القاع في `useHeaderCollapse` — حلقة تغذية راجعة لا «اهتزاز طفيف»

الهيدر `sticky` أي **داخل تدفّق** الصفحة، فتقلّصه يُنقص ارتفاع المستند نحو ٣٠ بكسل. وحين يكون المستخدم ملتصقاً بأسفل الصفحة تماماً يضطر المتصفح لتقليل `scrollY` بنفس المقدار ليبقى ضمن الحدّ — فيصل ذلك كحدث تمرير «لأعلى» يتجاوز `DIRECTION_THRESHOLD_PX`، فيتمدّد الهيدر، فيزيد ارتفاع المستند، فيتمدّد التمرير… بلا نهاية.

**قِيس فعلياً لا افتُرض:** على شاشة جوال (390×844) تبدّل ارتفاع الهيدر بين 68px و98px **خمساً وثلاثين مرة في ثانيتين**. وPlaywright رفض النقر على زرّ «المزيد» في الهيدر بحجّة «element is not stable» — وهو ما كشف العطل أصلاً. العطل سابق لإعادة تصميم الشاشة (قِيس على الفرع الأساسي بنفس الأرقام)، لكنه صار مؤذياً حين استضاف الهيدر التنقّل الثانوي كلّه.

`DIRECTION_THRESHOLD_PX` لا يعالجه: الانزياح ٣٠ بكسل، أي أكبر من أي عتبة معقولة. العلاج: عند الالتصاق بالقاع لا تُبدَّل الحالة إطلاقاً — تبقى كما هي، وأي تمرير حقيقي لأعلى يُبعد المستخدم عن القاع فوراً فيعود السلوك المعتاد بلا تأخير محسوس.

⚠️ **اختبارات jsdom القائمة لم تكن لتلتقطه ولو كُتبت مئة**: jsdom يترك `scrollHeight` صفراً فلا يُفعَّل الحارس أصلاً (`maxScrollY` سالب) — ولهذا بقيت الستة القديمة خضراء قبل الإصلاح وبعده. الاختباران المضافان يضبطان الهندسة صراحةً (`scrollHeight`/`innerHeight`) ليقعا في نطاق الحارس فعلاً، وجُرِّبا بحذف الحارس والتأكد من سقوطهما.

### 🆕 حشو أسفل الصفحة `pb-24` في كل العروض — لا `md:pb-8`

`SmartInputBar` ثابت أسفل الشاشة عند **كل** عرض (يتغيّر عرضه ومركزه عند md، لا وجوده)، بينما كان حشو أسفل الصفحة ينكمش من 80px إلى 32px عند md فأكبر. النتيجة على 768×1024: آخر بطاقة مسافر تختفي 22 بكسل خلف الشريط — عطل سابق ظهر أثناء معاينة إعادة التصميم.

⚠️ **ويحرسه `e2e/bottom-bar-clearance.spec.ts` على أربعة عروض لا واحد**: العطل كان محصوراً بنقطة انكسار واحدة (md)، فاختبار الجوال وحده كان سيبقى أخضر إلى الأبد. والاختبار يتحقّق أولاً من أننا بلغنا قاع الصفحة فعلاً — وإلا فالقياس بلا معنى.

### 🆕 «إدارة الرحلات» حُذفت — «رحلاتي» تنقّل بحت، والتعديل عبر اسم الرحلة في الهيدر

كانت شاشة «إدارة الرحلات» (`TripAdminView.tsx`) وشاشة «رحلاتي» (`TripPicker.tsx`) تبدوان متطابقتين لصاحب حسابٍ هو مسؤول ومنظّم معاً (نفس الرحلات في كلتيهما)، لكنهما تختلفان في مصدر البيانات: **رحلاتي** تقرأ `useMyTrips` — مستندات فردية (`getDoc` لكل رحلة، تسمح بها القواعد لأي عضو عادي) — وهدفها التنقّل بلا تعديل. **إدارة الرحلات** تقرأ `useAllTrips` — استعلام قائمة كامل يرضيه `isAdmin()` وحده — وتحوي التعديل الكامل. عضو عادي لا يصل للثانية إطلاقاً؛ فالتشابه كان صدفة ظهرت فقط لحسابات تملك الصلاحيتين معاً. حُذفت الشاشة، ودُمج التعديل مباشرة في التطبيق بدل قائمة وسيطة.

**محاولة أولى مرفوضة: زرّ «تعديل» على كل بطاقة في «رحلاتي».** كانت الخطوة الطبيعية الأولى — نقل `TripDetailPanel` إلى داخل القائمة، وزرّ يفتحها لمن يملك الصلاحية. عملت فعلاً، لكن صاحب الحساب فضّل شيئاً أبسط: تعديل الرحلة *المفتوحة حالياً* مباشرة من الهيدر (`EditTripModal.tsx`، مودال كامل الشاشة يستدعي `TripDetailPanel` كما هي بلا تغيير) — بلا المرور بـ«رحلاتي» أصلاً. «رحلاتي» رجعت تنقّلاً بحتاً (فتح/إنشاء/استعادة)، ولا تعديل منها إطلاقاً.

**الأثر العملي الوحيد:** تعديل رحلة *أخرى* غير المفتوحة يحتاج خطوة إضافية — فتحها أولاً من «رحلاتي» (المسؤول يرى كل الرحلات هناك ويدخل أيّاً منها بصلاحية `isAdmin()`، بصرف النظر عن عضويته) ثم تعديلها بعد أن تصبح هي المفتوحة. لا قدرة فُقدت، خطوة واحدة زادت — تبادل قبله صاحب الحساب صراحة إذ كان يعرف أصلاً أن المسؤول يدخل أي رحلة بلا قيد.

⚠️ **محاولة ثانية مرفوضة، اكتشفها CI لا المراجعة اليدوية: زرّ التعديل داخل اسم الرحلة نفسه.** بدا منطقياً — «اضغط اسم الرحلة لتعديلها» — لكن اسم الرحلة يختفي كلياً حين يتقلّص الهيدر (`useHeaderCollapse`، عند التمرير لأسفل) ويحلّ محلّه ملخّص الإحصاءات. اختبار `delete-empty-trip.spec.ts` كشف هذا فعلياً: الهيدر تأرجح بين الحالتين أثناء استقرار الصفحة بعد إنشاء الرحلة، فتأرجح الزرّ معه بين الظهور والاختفاء حتى انتهت مهلة الضغط (30 ثانية) دون استقرار.

**محاولة ثالثة مرفوضة أيضاً: أيقونة قلم مستقلة بجانب «حسابي».** أصلحت مشكلة الاختفاء (ثابتة بصرف النظر عن تقلّص الهيدر)، لكن صاحب الحساب رفضها من زاوية مختلفة: عنصر إضافي يزحم هيدراً مزدحماً أصلاً (شعار، اسم/إحصاءات، AccountMenu). محاولة رابعة (شعار التطبيق نفسه بشارة قلم صغيرة عليه) رُفضت أيضاً — لا حاجة لأي شارة بصرية إطلاقاً.

**الحل النهائي: الشعار نفسه هو الزرّ، بلا أي شارة أو أيقونة إضافية.** وحين لا يكون الهيدر متقلّصاً، اسم الرحلة يدخل ضمن نفس عنصر الضغط (الشعار واسم الرحلة معاً داخل زرّ واحد) — لا عنصرين منفصلين بنفس التسمية معاً، فيبقى استعلام واجهة واحد فقط (`getByRole('button', { name: 'تعديل الرحلة' })`) صحيحاً دائماً. في وضع التقلّص (حين يختفي الاسم) يبقى الشعار وحده هو الزرّ. إعادة استخدام عناصر قائمة أصلاً (الشعار، اسم الرحلة) بدل أي عنصر جديد يحلّ الاختفاء والازدحام معاً بلا مفاضلة بينهما.

⚠️ **وأثناء المراجعة: الرحلات المؤرشفة كانت غير قابلة للوصول إطلاقاً بلا رابطها المباشر.** فلتر «رحلاتي» القديم (`t.status !== 'archived' || t.id === TRIP_ID`) ورثته هذه الشاشة الموحّدة أيضاً — فحتى المسؤول، الذي يُفترض أن يدخل أي رحلة في النظام، لم يعد يرى الرحلات المؤرشفة في قائمته أصلاً ليدخلها. **الحل: صفّ «الرحلات المؤرشفة» القابل للطي أسفل القائمة النشطة** (نمط «الدردشات المؤرشفة» في واتساب) — لا تختفي نهائياً، ولا تزدحم يومياً مع القائمة النشطة. `useAppCoordinator.ts` يقسم القائمة إلى `trips`/`archivedTrips` بدل إسقاط المؤرشفة صامتاً.

⚠️ **فخّ الأداء الذي كاد يمرّ مرّتين:** كل من `TripAdminView` القديمة و`TripDetailPanel` داخلها كانا جزءاً كسولاً (`lazy()`) — لا يصلان حزمة عضو لا يفتح الإدارة إطلاقاً. أول نقل لـ`TripDetailPanel` داخل `TripPicker.tsx` (غير كسول، يُحمَّل عند كل بدء تشغيل) كاد يُدخله الحزمة الرئيسية دائماً؛ أُصلح بإبقائه كسولاً *داخل* `TripPicker` حينها. وحين انتقل التعديل إلى `EditTripModal.tsx` (مودال ضمن `ModalManager.tsx`)، بقي الدرس نفسه: كسول هناك أيضاً (`importEditTripModal` في `modalImporters`)، فظهر `EditTripModal-*.js` (~30 ك.ب) كجزء منفصل تماماً، والحزمة الرئيسية لم تتأثر.

### 🆕 سهم «المزيد» ملازمٌ للشعار — لأن `line-clamp-2` كانت تبتلعه

سهم ▾ بعد اسم الرحلة هو **الدليل الوحيد** على أن العنوان قائمة تُفتح (وهذا مسجّل في هذا الملف نفسه). وكان `inline` داخل الـ`<h1>` ليتبع آخر كلمة أينما وقعت — لكن `line-clamp-2` تقصّ **محتوى الصندوق كله** عند سطرين وتضع «…»، فتبتلع السهم معه.

**النتيجة: اسمٌ طويل كان يُخفي «المزيد» كلها** — التقارير والإحصائيات والمسار وإدارة الرحلة — صحيحةً وغير قابلة للاكتشاف في آنٍ واحد. قِيس فعلياً على 360 و375 بكسل باسم «رحلة الكادر التعليمي لمدارس الرياض الأهلية 2026».

⚠️ **وهو عيب سابق لزرّ «رحلاتي» لا ناتج عنه** (يتكرّر بحذف الزرّ)، لكن الزرّ يقتطع ~٥٦ بكسل من عمود الاسم فصار يقع عند أسماء أقصر بكثير.

**ومحاولتان سقطتا قبل الحلّ:**
1. **سهم شقيقٌ بعد العنوان** — يُدفع إلى أقصى اليسار ملاصقاً لقائمة الحساب فيُقرأ كأنه جزء منها (وهذا الاعتراض مسجّل هنا من قبل، وما زال صحيحاً).
2. **حجز مساحة تمنع ذلك (`pe-7` على عمود الاسم)** — يعمل، لكنه يقتطع ٢٨ بكسل من عمودٍ عرضه **١٣٦ بكسل** أصلاً على شاشة 360، فيبقى للاسم **٥٠ بكسل**. قِيس بـ`getBoundingClientRect`، لا يُعاد.

**الحلّ: السهم ملازمٌ للشعار داخل نفس الزرّ** (`<span>` يضمّ `Logo` ثم `ChevronDown`). وهو ليس حلاً وسطاً بل **توحيد**: الحالة المتقلّصة تفعل هذا بالضبط منذ البداية. فصار السهم في موضع واحد لا يتنقّل مع تمرير الصفحة، ولا يُقصّ مهما طال الاسم، ولا يكلّف الاسم بكسلاً واحداً (عرض الاسم على 360 ارتفع من ٥٠ إلى ٨٢ بكسل).

**والثمن المقبول:** اسم بطول «رحلة بولندا 2026» صار يلتفّ لسطرين على 375 بكسل حين يظهر زرّ «رحلاتي» (ينقصه ٣١ بكسل لسطر واحد). `line-clamp-2` مصمَّمة لهذا أصلاً، والحجم الرأسي للكتلة محجوز سلفاً.

### 🆕 بعد الدخول لا «تجربة حساب»: دخول ← رحلاتي ← الرحلة

المسار عند التوجيه (`App.tsx`) كان صحيحاً أصلاً — `AuthGate` ثم «رحلاتي» ثم الرحلة، بلا خطوة بروفايل إجبارية. لكن **«رحلاتي» نفسها كانت بنداً داخل قائمة الحساب**، فالرجوع لقائمة الرحلات يمرّ بـ«حسابي»: أي `دخول ← حساب ← … ← رحلات` حرفياً، وهو ما رفضه صاحب الحساب.

**«رحلاتي» زرٌّ مستقلّ يتصدّر الهيدر الآن**: نقرة واحدة، نمط زرّ الرجوع من المحادثة إلى قائمة المحادثات. ولا يظهر إطلاقاً لمن له رحلة واحدة — لا شيء يُرجَع إليه. القاعدة التي تُصنّف: **هل هذا حساب أم مسار؟** المسار لا يسكن قائمة الحساب.

⚠️ **وموضعه في الصفّ ليس تفصيلاً.** أول نسخة وضعته **بجانب الأفاتار** في طرف الهيدر، فردّها صاحب الحساب: الترتيب المنطقي هو `[رحلاتي] [أيقونة الرحلة واسمها] … [الحساب]`. وهو محقّ بمعيار قابل للتطبيق لا بذوق: زرّ الرجوع يتصدّر شريط العنوان في كل تطبيق محادثة لأنه **يسبق ما تنظر إليه** — أنت خارجٌ من هنا إلى القائمة. ووضعُه ملاصقاً للأفاتار يجعله يُقرأ كأنه من عناصر الحساب، وهو بالضبط الالتباس الذي نُقل من قائمة الحساب ليتخلّص منه — فنقلُه كان سيصير حركةً بلا أثر.

⚠️ وهذا يخالف ظاهرياً رفضاً سابقاً في هذا الملف نفسه (زرّ تعديل مستقلّ في الهيدر، رُفض للازدحام) — والفرق جوهري: ذاك كان فعلاً *ثانوياً* له بديل مجاني (الشعار نفسه)، وهذا هو المسار *الرئيسي* الذي تُقاس به الشاشة كلها. مساحة الهيدر تُنفَق على ما يُستعمل يومياً.

**وقائمة الحساب صارت شيئين: من أنت (تفتح بروفايلك)، وتسجيل الخروج.** كانت أربعة.

⚠️ **و«الدخول بحساب آخر» حُذف بنافذته وخطّافه كاملاً** — لا البند وحده. كان يجرّ خلفه `AdminSignInModal` (نموذج بريد/كلمة مرور ثانٍ)، و`AuthFlow` (غلاف عرضه)، و`useAdminAuth` (خطّاف مصادقة ثانٍ). ثلاثة ملفات و**سطح تسجيل دخول ثانٍ كامل** لفعلٍ يؤدّيه الخروجُ ثم الدخول بالحساب الآخر — وكان يُعرض لغير المسؤول وغير المنظّم حصراً، أي لمن يملك حساباً واحداً غالباً. صار في التطبيق سطح دخول واحد: `AuthGate`. و`signOut` انتقل إلى `useAuth` حيث يعيش الدخول — خطّاف مصادقة واحد لا اثنان.

**ما لم يتغيّر:** Google + Email/Password كما هما (المسار الاحتياطي مهمّ)، و«نسيت كلمة المرور؟» في `AuthGate` كما هو، وخطوة الاسم بعد رابط الدعوة (`InviteJoinScreen`) كما هي — شرطية ونادرة وقابلة للتخطّي.

⚠️ **وفخّ اختبار كشفه النقل:** `openTripAsAdmin` في `e2e/utils/flows.ts` كانت تتأكّد من وصول المسؤول بفتح قائمة الحساب وانتظار بند «رحلاتي» فيها — أي أن **كل اختبار E2E يمرّ بحساب مسؤول** كان يعتمد على موضع ذلك البند. نقلُه إلى الهيدر أسقط ١٩ اختباراً دفعةً واحدة، لا اختبار «رحلاتي» وحده. هذا ليس عيباً في الاختبارات بل فائدتها: مساعِدٌ مشترك يمرّ به الجميع يجعل تغيير موضع عنصر مرئياً فوراً بدل أن يمرّ صامتاً.

### 🆕 بطاقة «رحلاتي» أربعة أشياء لا خامس — والرقمان تجميعٌ خادمي لا قراءة مجموعات

البطاقة كانت تعرض: الاسم، والمعرّف (للمسؤول)، و**ملخّص المسار بتواريخه** («الرياض ← دبي · ١٢ يوليو – ١٨ يوليو»)، والحالة. صاحب الحساب ردّها لأربعة: **الاسم، عدد المسافرين، إجمالي المصروف، الحالة** — «لا تحوّل Trip Picker إلى dashboard آخر».

**والمفارقة المقصودة: الحذف والإضافة في نفس الخطوة.** المسار حُذف لا لأنه كثير بل لأنه **معلومة داخل الرحلة تُعرض قبل دخولها**؛ من يقف أمام القائمة يسأل «أيّ رحلة أفتح؟» لا «متى كانت». وعدد المسافرين والإجمالي أُضيفا لأنهما الجوابان الوحيدان على ذلك السؤال — حجم الرحلة ومقدارها. العدد أربعة لا لأنه سقف جمالي، بل لأن كل زائد عليه يجيب عن سؤال يُطرح *بعد* الفتح لا قبله.

**والمعرّف ذهب معه — ولا استثناء للمسؤول.** كانت الحجّة لإبقائه أنه ليس معلومة عرض بل ما يميّز رحلتين باسم واحد لمن يتصفّح رحلات لا يعرفها. ردّها صاحب الحساب: الأربعة أربعة، والاستثناء «للمسؤول وحده» هو بالضبط ما يُعيد البطاقة لوحةَ معلومات على مهل. ومن احتاج تمييز رحلتين متشابهتين يملك الحلّ الصحيح أصلاً — أن يسمّيهما باسمين مختلفين.

⚠️ **وحذفه كشف اختباراً كان سينقلب أخضر كاذباً:** `tripRowInPicker` في `e2e/utils/flows.ts` كانت تطابق الصفّ **بمعرّفه**، وتعمل فقط لأن الصفّ يعرضه للمسؤول. بعد الحذف صار المحدِّد لا يطابق شيئاً — وهو مستخدَم في تحقّق **سلبي** (`toHaveCount(0)` في `delete-empty-trip.spec.ts`: «الرحلة اختفت بعد حذفها»). أي أن الاختبار كان سينجح مجاناً سواء حُذفت الرحلة أم لا. صارت الدالة تأخذ **الاسم المعروض** صراحةً، ويستعملها اختبار الإحصاءات بتحقّقات **موجبة** فتُثبت أنها تطابق صفّاً موجوداً فعلاً — وهو ما لا يستطيع أي `toHaveCount(0)` إثباته عن نفسه.

⚠️ **ولماذا `getAggregateFromServer` لا `getDocs` ثم `calculateTotalSpent`؟** الطريق المغري هو إعادة استخدام الحساب الموجود: اقرأ المصاريف واجمعها محلياً كما تفعل الترويسة. وهو خطأ **هنا تحديداً** لأن «رحلاتي» شاشة *الدخول*: تكلفة قراءة كل مصاريف كل رحلة تنمو مع **طول الرحلة** لا مع عدد الرحلات، ورحلة طويلة بدورات شهرية (`closeMonth` يكتب مصاريف ترحيل حقيقية) تبلغ آلاف المستندات — تُقرأ كلها قبل أن يرى المستخدم أسماء رحلاته. التجميع خادمي ويُحاسَب بقراءة واحدة لكل ١٠٠٠ مستند مطابق، فتكلفته ثابتة عملياً.

⚠️ **والفخّ الذي يستحقّ التسجيل: `where('deletedAt', '==', null)` كان سيكذب صمتاً.** الحذف ليّن، و`deletedAt` **قد يكون غائباً تماماً** من المستندات القديمة (`isValidExpense` في `firestore.rules` تقبل `!('deletedAt' in d)`). واستعلام المساواة بـ`null` لا يطابق مستنداً لا يملك الحقل أصلاً — أي أن كل مصروف قديم كان سيسقط من الإجمالي، فيظهر رقم أقلّ من الحقيقة بلا أي خطأ ظاهر. الحلّ الصحيح **الطرح**: المجموع الكامل ناقص مجموع المحذوف، بفلتر `deletedAt > 0` — ومقارنات المدى في Firestore لا تطابق إلا القيم من نفس النوع، فلا `null` ولا حقل غائب يدخل فيها. استعلامان لكل مجموعة بدل واحد، وهو الثمن الصحيح.

والقاعدة الملزمة: **رقم البطاقة يجب أن يساوي ما تعرضه ترويسة الرحلة بعد فتحها** (`activeExpenses`/`activeTravelers` ثم `useBalances`). أي فرق هو تناقض مالي يراه المستخدم على شاشتين متتاليتين.

**والفشل صامت بالتصميم.** التجميع خادمي بحتاً ولا يعمل من الكاش المحلي، فهو يفشل دائماً بلا اتصال. حينها تبقى الرحلة خارج خريطة الإحصاءات وتُرسم بطاقتها بالاسم والحالة وحدهما — أي بطاقة ما قبل هذا التغيير بالضبط. لا رسالة خطأ ولا هيكل تحميل على رقم ثانوي لا يمنع أحداً من فتح رحلته.

**والإجمالي بلا هللات وبفواصل آلاف** (`12,480 ﷼`) بخلاف `toFixed(2)` في الترويسة والتسويات: هذا رقم *ملخّص* يُقرأ بلمحة لا مبلغ تُسوّى به ديون. الرقم بالهللة يبقى حيث يُحتاج فعلاً — داخل الرحلة وفي التقارير والطباعة وExcel.

### 🆕 عنوان الهيدر يجيب عن «أين أنا» لا عن «ما هذا التطبيق»

العنوان كان «مصاريف السفر» ثابتاً — يجيب عن سؤال لا يسأله أحد وهو داخل التطبيق. صار يعرض **اسم الرحلة المفتوحة**، نصّاً لا زرّاً. اسم التطبيق باقٍ في `index.html` (تبويب المتصفح وPWA) وهو مكانه الصحيح.

ومع تعدّد الرحلات وشاشة «رحلاتي»، صار تأكيد *أي رحلة مفتوحة* قبل تسجيل مصروف فيها **معلومة مالية لا ترفاً بصرياً**. رحلة بلا اسم محفوظ تعرض معرّفها، لا اسماً مخترعاً.

⚠️ وأثره على الاختبارات ليس تجميلياً: أي بحث نصّي عن اسم رحلة صار يطابق عنصرين — الهيدر، وحيثما ظهر الاسم في لوحة الإدارة — فوقعت `delete-empty-trip.spec.ts` في strict mode. والأسوأ أن التحقّق من *اختفاء* رحلة محذوفة صار سباقاً زمنياً: الهيدر يُسقِط الاسم إلى المعرّف بعد أن يصل الحذف إلى `onSnapshot`. الحلّ كان التحقّق **بمعرّف الرحلة** الذي يظهر في القائمة وحدها، فيزول السباق من أصله لا يُخفَّف.

### 🆕 «تصدير Excel» حُذف من شريط سجل المصاريف، و«سلة المهملات» نزلت إلى نهايته

**التصدير لم يكن نقطة دخول ثانية لميزة، بل الزرّ ذاته مرتين**: استدعاء حرفي لـ`exportTripToExcel` بنفس الوسائط الأربع التي يستدعيها الزرّ داخل `ReportsView`. حُذف بلا بديل — مكانه الطبيعي داخل التقارير حيث يُنظَر إلى ما يُصدَّر، والتصدير فعل نادر *ومخطَّط له* لا يخسر شيئاً بعمق نقرة. (`e2e/critical-flow.spec.ts` كان يختبر زرّ التقارير أصلاً لا زرّ الشريط.)

**والسلة نزلت إلى نهاية السجلّ ولم تنتقل إلى إعدادات الرحلة.** طُرح نقلها إلى لوحة إدارة الرحلة فرُفض: السلة **مسار تعافٍ من خطأ لا إعداد**، ومسارات التعافي تسكن حيث يقع الخطأ — من حذف مصروفاً بالخطأ يكتشف ذلك وهو ينظر إلى السجلّ، وفي لحظة قلق. وقاعدة «النادر يربح بالإخفاء» لا تنطبق عليها: التصدير نادر ومخطَّط له، والاستعادة نادرة **وطارئة**. ولها في المكان نفسه طبقتا تعافٍ متدرّجتان — «تراجع» الفوري في التنبيه ثم السلة لمن فاته وقته — وفصلهما يكسر تدرّجاً يعمل.
...
### 🆕 `useSyncRecovery`: a fresh server read on resume — because the "silent" disconnect is the one the SDK cannot see

**The complaint was "sync is sometimes slow", and the first useful move was eliminating whole families of causes rather than theorising about one** (guideline 22). Ruled out by inspection and measurement, in this order: the write path (no `runTransaction` anywhere — every write is `writeBatch`/`setDoc`, applied to the local cache synchronously), the service worker (`*.googleapis.com` is `NetworkOnly` in `vite.config.js`, so it never touches sync traffic), and listener churn (`hasAccess` is a boolean and `user`'s reference is stable, so `useExpenses`/`useTravelers` never re-subscribe).

**Then the measurement that redirected the whole investigation: the clean disconnect is not the problem and never was.** A Playwright test that took the browser offline, wrote an expense from a second device via the Admin SDK, and came back online saw the change in **93 ms**. The Firestore SDK listens for `online` itself and resumes immediately. Any fix aimed at offline/online events would have been aimed at the one case that already works.

**The real cost is in `persistentMultipleTabManager`: exactly one tab owns the Firestore network stream, and every other tab reads through IndexedDB.** Measured on the emulator: **32 ms** for a secondary tab while the primary is alive, versus **4398 ms** after the primary dies abruptly — a 137× difference, and that is on a desktop machine over loopback. The gap is the primary-lease expiry that must elapse before another tab can take over.

**Why that number describes a phone rather than a laboratory:** killing the stream-owner without a clean handoff is precisely what a mobile OS does to a backgrounded PWA tab — it freezes it or discards it. And a *frozen* primary is worse than a closed one: it still holds its lease and cannot renew it, and on thaw it still believes it owns a socket the OS already destroyed. None of this raises an `offline` event, so none of it reaches the SDK's own recovery path.

**The structural gap was that the app had no resume path at all** — `visibilitychange`, `pageshow` and `enableNetwork` returned zero hits across `src/`. The irony is that `useAuth` had already learned this exact lesson and carries `window.addEventListener('online', onlineRetry)` for claims; the data listeners simply never got the same treatment. The only manual recovery was pull-to-refresh, which is a pure touch gesture (`onTouchStart`) that only arms at `window.scrollY === 0` — so it does not exist on desktop, and elsewhere it requires the user to already suspect the data is stale, which is the one thing a silent staleness bug guarantees they cannot know.

**Why a read and not a retry queue.** "Optimistic updates roll back automatically — do not build a retry queue" (above) is about *writes*, and this hook does not touch them: it holds no payload, re-sends nothing, and cannot resurrect a rejected write. It issues one `getDocsFromServer` at a moment when the connection has plausibly died unannounced. The two mechanisms don't overlap.

**The guards are the design, not defensive padding.** A 10-second cooldown, seeded from *mount* rather than zero (the snapshot listeners have just done their first read, so a recovery in the seconds after mount is pure duplication); an in-flight lock, because `visibilitychange` and `online` routinely fire together; a `visibilityState === 'visible'` check, because a network return while the app is backgrounded is worth nothing — the subsequent `visibilitychange` covers it when the user actually looks. And silence on failure: the live listener is still the primary path, and an automatic action the user did not request should not produce an error toast, which is exactly why pull-to-refresh *does* report its failures and this does not. **Each of those four guards was verified by deleting it and confirming a specific test failed** (guideline 18) — not by reading the code back.

### 🆕 One shared `isSyncing` flag between two independent listeners made the badge lie

`useExpenses` and `useTravelers` were both handed the *same* `setIsSyncing`. Each raised it on subscribe and lowered it inside its own snapshot handler, so whichever collection answered first cleared the badge while the other was still on its first read — and because both subscribe with `includeMetadataChanges: true`, a metadata-only event from either one cleared it too.

**This is worth recording as more than a cosmetic slip, because of how it interacts with the entry above.** A stale-data bug is tolerable when the interface admits it and infuriating when the interface denies it. The badge reading "synced" over genuinely old data is what turns a bounded, explicable delay into something the user experiences as random breakage — and it is also why the original report was "sometimes", with no reproducible pattern to hand back. Each listener now owns its own flag and `isSyncing` is derived (`a || b`), which makes the wrong-listener-clears-it failure unrepresentable rather than merely fixed.
