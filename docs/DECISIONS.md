...
### 🆕 «رحلاتي» و«إدارة الرحلات» شاشة واحدة الآن — لا تكراراً بصرياً بل ملكية بيانات مختلفة أُخفيت خلف واجهتين متشابهتين

كانتا تبدوان متطابقتين لصاحب حسابٍ هو مسؤول ومنظّم معاً (يرى نفس الرحلات في كلتيهما)، لكنهما لم تكونا كذلك بنيوياً: **`TripPicker` (رحلاتي)** يقرأ `useMyTrips` — مستندات فردية (`getDoc` لكل رحلة، لا استعلام قائمة) تسمح بها القواعد لأي عضو عادي — وهدفه التنقّل بلا تعديل. **`TripAdminView` (إدارة الرحلات، حُذفت)** يقرأ `useAllTrips` — استعلام قائمة كامل يرضيه `isAdmin()` وحده — ويحوي التعديل الكامل. عضو عادي لا يصل لـTripAdminView إطلاقاً؛ فكان التشابه صدفة ظهرت فقط لحسابات تملك الصلاحيتين معاً.

الدمج ممكن تحديداً لأن **`TripAdminView` لا يُفتح أصلاً إلا لمسؤول أو منظّم** (`useAppCoordinator.ts`: `viewerRole: isAdmin ? 'admin' : 'organizer'`)، ولأن بيانات المنظّم لرحلته الخاصة كانت متاحة له من قبل بلا استعلام قائمة (نفس أساس `useMyTrips`) — فقط لم تُقرأ بها الحقول الكاملة (`itinerary`/`organizerUid`/`statusChangedAt`). التوسيع كان مجانياً: التوسيع لا يكشف شيئاً لا يملك العضو صلاحيته أصلاً (نفس المستند، نفس `isMember()`).

**زرّ «تعديل» يظهر على أي بطاقة يملك المستخدم صلاحية تعديلها** — المسؤول على أي رحلة، والمنظّم على رحلاته هو وحدها (`organizerUid === uid`، وليس فقط الرحلة المفتوحة حالياً: `organizerTripIds` في `useAppCoordinator.ts` صارت مصفوفة لا قيمة واحدة). يفتح نفس `TripDetailPanel` الذي كان في اللوحة القديمة حرفياً — لم يتغيّر سطر فيه. المسؤول وحده يرى معرّف كل رحلة تحت اسمها (يتصفّح رحلات لا يعرفها بالاسم فقط، وقد تتشابه الأسماء)؛ عضو عادي يرى رحلاته القليلة المعروفة له بالاسم وحده.

⚠️ **فخّ الأداء الذي كاد يمرّ:** `TripAdminView` كانت جزءاً كسولاً (`lazy()`) منفصلاً — المكوّنات الثقيلة (`TripDetailPanel` بتبويباتها الست، `RestoreTripForm`) لم تكونا تصلان الحزمة الرئيسية إطلاقاً لعضو لا يفتح الإدارة. أول نقل ساذج لهما داخل `TripPicker.tsx` (غير كسول أصلاً، يُحمَّل عند كل بدء تشغيل) كان سيُدخلهما حزمة كل مستخدم دائماً. البقاء كسولَين *داخل* `TripPicker` (مع `tripPickerImporters` مُصدَّرة للتحميل المسبق الهادئ، بنفس نمط `modalImporters`) هو ما أبقى `dist/assets/index-*.js` بنفس الحجم تقريباً بعد الدمج، مع ظهور `TripDetailPanel-*.js` (٢٩ ك.ب) كجزء منفصل كما كان قبل الحذف.
