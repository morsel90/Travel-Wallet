...
### 🆕 منظّم الرحلة مشرفها الكامل — والمسؤول العالمي للخلاف وحده

**السبب:** أول منظّم حقيقي أنشأ رحلته ولم يستطع حذف مسافر أضافه بالخطأ ولا تسجيل مودَع، لأن `travelers.update` و`depositLogs.create` كانتا `isAdmin()`. والمسؤول العالمي لا يحضر الرحلة، فلا يعرف متى تنتهي ولا من خرج منها. صاحب القرار حدّد الأدوار: **المنظّم يدير رحلته كاملةً، والمسؤول يُستدعى وقت الخلاف فقط.** فصار السؤال: ما الذي يحتاجه المسؤول ليحكم في خلاف؟ والجواب: **الأثر**. كل صلاحية أُعطيت للمنظّم مشروطة بألّا تمحو أثراً:

| الصلاحية | الطريق | ما يبقى محمياً |
|---|---|---|
| حذف مسافر ليّناً واستعادته | القواعد: `affectedKeys().hasOnly(['deletedAt'])`، وحذف حجز الاسم | لا يلمس الاسم ولا `uid` ولا الرصيد مباشرة |
| المودَع (إضافة/خصم/تحديد) | دالة `recordDeposit` | الرصيد وسطر `depositLogs` في معاملة واحدة، و`changedByUid` هو المستدعي الفعلي |
| تعديل مصروف غيره وحذفه | القواعد: `isOrganizer(appId) && stampsEditor(...)` | «عدّله فلان» إلزامي، ولا يحمل إلا هوية الكاتب (`editorStampIsHonest` على الجميع) |
| سلة المهملات | إظهارها له | — |

**لماذا دالة للمودَع وقاعدة للحذف؟** القاعدة تستطيع أن تحصر الكتابة في حقل `deletedAt` وحده. لكنها **لا تستطيع** أن تشترط وجود سطر في `depositLogs` مع كل تغيير في `deposited`، لأن معرّف السطر عشوائي. فلو وُسّعت القاعدة للمنظّم لملك تعديل الرصيد بلا أثر، وهو بالضبط ما تحذّر منه فقرة «لماذا دالة سحابية لا توسيع القواعد» في قرار التسوية أدناه. المسؤول يبقى على الدفعة المحلية لأنها تعمل بلا اتصال، والمنظّم يمرّ من الخادم (`useDepositActions`، `viaServer`). والدالة نسخة حرفية من `applyDepositMode`: الخصم يُقصَر عند الصفر، ولا تقريب.

**«عدّله فلان» لم يكن ضرورياً قبل اليوم.** كان المسؤول وحده يعدّل مصاريف غيره، وهو الحَكَم نفسه. صار ضرورياً حين صار المنظّم، وهو طرف في الرحلة، يملك التعديل. الختم يُكتب مع كل تعديل وحذف واستعادة من أي أحد، لأن ذلك أبسط وصادق، ويُعرض فقط حين يختلف المعدِّل عن كاتب المصروف. هو ليس سجلّ تاريخ كاملاً، فآخر تعديل وحده يظهر. التاريخ الكامل قرار لم يُتّخذ بعد.

**المنظّم المساعد، حتى لا تتوقف الرحلة إن غاب منظّمها.** «منظّم واحد لكل رحلة» (انظر `manageMember`) قيدٌ على **`organizerUid`** لا على الدور. `organizerUid` يحدّد بيانات البنك المعروضة ويجب أن يبقى قيمة واحدة. أما الصلاحيات فتقرأ `role` وحده (`isOrganizer()` في القواعد و`callerManagesTrip` في الدوال). فحين يعيّن المنظّم مساعداً يُكتب `role: 'organizer'` على سطر عضويته فقط: لا `organizerUid`، ولا خفض لأحد، ولا `organizesTripIds`. والمساعد يُعرض «منظّم مساعد» لأن `uid` الخاص به لا يساوي `organizerUid`. لا يغيّر المنظّم دور المنشئ ولا دوره هو، فنقل الملكية فعلاً قرار المسؤول، وتعيينه بقي كما كان.

**حذف الرحلة لمنشئها وحده (2026-09-25):** `manageTrip` (`mode: 'delete'`) يقبل المسؤول أو صاحب `organizerUid`، ولا يقبل المنظّم المساعد. الحذف نهائي ويسحب وصول كل المسافرين، فيبقى قرار صاحب الرحلة. شروط الأهلية لم تتغيّر وتسري على الجميع: رحلة فارغة، أو منتهية ومسوّاة، أو قديمة مؤرشفة. وتعيين المسؤول لمنظّم نقلُ ملكية، فالمنظّم المعيَّن هكذا يحذف أيضاً.

**ما بقي للمسؤول عمداً:** النسخ الاحتياطي والاستعادة، ونقل ملكية الرحلة (تعيين المنظّم الأصلي).

⚠️ **كُشف أثناء هذا العمل خلل قديم منذ أول commit:** `useTravelers` كان يستبعد المحذوفين في الـ query نفسه (`where('deletedAt', '==', null)`). فكان تبويب «المسافرون المحذوفون» في السلة **فارغاً دائماً لكل أحد**، والمسؤول منهم، وكانت «استعادة»/«تراجع» تبحث عن المسافر في قائمة لا تحويه فتخرج بصمت. `useTripWorkspace` كان يقسم القائمة إلى نشطين ومحذوفين بنفسه من البداية، والمتجر لا يستلم إلا النشطين، فرفع الفلتر لا يغيّر شيئاً مما يُعرض أو يُحسب. كشفه `e2e/organizer-full-management.spec.ts` حين حاول الاستعادة من السلة.

---

Decisions that look like oversights but are not. Read this before "fixing" them.

### 🆕 حذف الرحلة يُزيل الوصول إليها — من كل الحسابات لا من سجلّ الأعضاء

**الـclaim هو العضوية** (`isMember()` تقرؤه وحده، بلا `get()`). فحذف البيانات وحده يترك كل حامل للعضوية «عضواً» في معرّفٍ فارغ. والحذف يُعلن المعرّف متاحاً لرحلة جديدة، فتُفتح تلك الرحلة لأعضاء القديمة فوراً.

**لماذا مسح كل الحسابات:** سجلّ `trips/{id}/members` أحدث من العضوية نفسها، وحسابات حقبة رمز الرحلة لا أسطر لها فيه. عند حذف travelapp-87206 كان في السجلّ صفر، وفي الـclaims 218. المسح بـ`listUsers` كلفته حسابٌ لكل حساب مسجّل، وهو هيّن في حجم هذا المشروع وفي فعلٍ نادر كالحذف. وإن كبر المشروع يوماً فالبديل مهمة خلفية، لا الرجوع إلى السجلّ الناقص.

**ولماذا بعد حذف البيانات ولا يُفشله:** الحذف هو ما طُلب. وبقايا الوصول إلى معرّف فارغ لا تكشف بيانات، وتُنظَّف بالسكربت. أما عكس الترتيب فقد يترك أعضاءً بلا وصول إلى رحلة ما زالت قائمة.

**و«حُذفت» من الخادم وحده:** لقطة كاش بلا اتصال قد لا تحمل المستند بعد. إعلان الحذف منها يطرد المستخدم من رحلة حقيقية، والانتظار أهون. ولهذا `includeMetadataChanges`: تأكيد الخادم لما قاله الكاش تغيّرٌ في الـmetadata وحدها.

### 🆕 رحلة منتهية ومسوّاة تُحذف فوراً — الـ٩٠ يوماً ليست الشرط، بل ما تحميه

**بلاغ صاحب المشروع:** رحلة يوم واحد (Bh26) سُوّي حسابها بتحويل بنكي في مساء اليوم نفسه. رفض التطبيق حذفها، ولم يكن هناك مسار إلا الانتظار: ٣٠ يوماً حتى الأرشفة ثم ٩٠ يوماً مؤرشفة — نحو أربعة أشهر لرحلة لم يبقَ فيها ريال مستحق.

**المسار الثالث (`settledClosureBlock`):** رحلة **ليست نشطة** و**كل أرصدة مسافريها صفر** تُحذف فوراً، ومعها `artifacts/{tripId}` كاملاً — تماماً كمسار العمر. الـ٩٠ يوماً تحمي من **حذف دين لم يُدفع**، ورحلة أرصدتها صفر لا دين فيها أصلاً؛ فالشرط الجديد يحمي الشيء نفسه مباشرة بدل أن يقيس الزمن كوكيل عنه.
- **«ليست نشطة»:** رحلة جارية قد يُضاف إليها مصروف بعد دقيقة فيعود الدين.
- **«صفر»:** الهللة نفسها (`ROLLOVER_EPSILON`) التي تعتبر بها الواجهة الحسابات «مصفّاة»، فما تقول الشاشة إنه مسوّى هو ما يقبله الخادم.
- **الأرصدة تُحسب في الخادم** من الدفتر (`readLedger`) لا من إدخال العميل: قرار حذف سجلّات مالية لا يعتمد على رقم أرسلته الواجهة.
- **مسار العمر باقٍ كما هو** للرحلة المؤرشفة غير المسوّاة (دينٌ تُرك عمداً ونُسي).

**والرسالة صارت تقول المخرج:** نشطة ← «غيّر حالتها إلى منتهية»؛ غير مسوّاة ← «سجّل التحويلات من الأرصدة». ولاحقة `[400]` كانت من SDK فايربيس نفسه، لا من الكود: يُلحق رمز HTTP بكل رسالة خادم، فكانت تسعة مواضع تعرض الرقم. `callableMessage` هو الموضع الوحيد الذي يعرف شكل الخطأ الآن.

### 🆕 التسوية تُسجَّل كحركة مالية — لا كعلَم «تم التحويل»

**العطل:** زرّ «تحديد كمُحوَّل» في `SettlementsPanel` كان يكتب في `useState` محلي. يُظلِّل السطر ويُظهر «تم التحويل ✓»، ثم يعود كل شيء كما كان عند إعادة التحميل، ولا يراه جهاز آخر. المستخدم يقرأ تأكيداً والدفتر لا يعرف شيئاً — **أسوأ من زرٍّ غائب**، لأن الغائب لا يَعِد.

**ولماذا لم يُحَلّ بحفظ العلَم في Firestore:** التسويات **مشتقّة** لا مخزّنة (`calculateSettlements` تُعيد توليدها من الأرصدة عند كل تغيّر)، فعلَمٌ على «فلان → فلان بمبلغ س» يتقادم مع أول مصروف جديد: يبقى التأشير معلّقاً على مبلغ لم يبقَ قائماً، ولا سبيل لمعرفة هل التأشير يعني «سُدّد كل الدين» أم «سُدّدت ١٠٠ من دين صار ١٤٥».

**ما يفعله `recordSettlement` بدلاً من ذلك:** يُطبّق التحويل على الطرف الدائن الوحيد في الدفتر (`deposited`) — المَدين دفع نقداً فصار كأنه أودع أكثر، والدائن استلم فصار كأنه أودع أقل — ويكتب سطرَي تدقيق (`add` للمَدين و`subtract` للدائن) في نفس المعاملة. **مجموع المُودَع لا يتغيّر**: تحويل بين طرفَين لا إيداع جديد.

🐛 **وكان فيها افتراضٌ خاطئ اكتُشف في الإنتاج:** أن الدائن دائنٌ بإيداعه. لكنّه في هذا الدفتر يصير دائناً أيضاً بدفع مصاريف **من جيبه** (`paidBy`)، وحينها يتجاوز رصيده ما أودعه، فينزل «المودَع» تحت الصفر بعد الخصم. حدث ذلك مرة واحدة (Bh26: ‎-170.5). والسالب يخالف `isFiniteAmount` (`>= 0`) في القواعد، ويكسر مطابقة `replayDepositLogs` (التي تقصر عند الصفر). **كان الحلّ المؤقت حارساً يرفض** بسبب مفهوم ولا يكتب شيئاً.

🆕 **والقرار اتُّخذ: قيدٌ ثالث (`repayments/`) لا مودَع سالب.** «مودَعٌ سالب بمعنى سحب من الصندوق» كان يُصلح الرقم ويُفسد المعنى: كل تقرير يعرض «المودَع» كان سيعرض ما ليس إيداعاً، و`applyDepositMode` تقصر عند الصفر عمداً، والقواعد تشترط `>= 0` في كل حقل مالي. أما القيد الثالث فيقول ما حدث حرفياً — «الدرويش سدّد لمحمد 170.5» — ولا يمسّ شيئاً آخر:
- **يرفع رصيد الدافع ويخفض رصيد المستلم** بالمبلغ نفسه، فمجموع الأرصدة ثابت (قاعدة ٧ في `calculations.invariants.test.ts`، 500 دفتر عشوائي لكل خاصية).
- **لا يمسّ `deposited` ولا `totalExpenses`**: لا «مودَع» مختلَق، ولا «إجمالي مصروف» منفوخ، ونصيب الدورة (`cycleShareBalances`) يبقى نصيباً حقيقياً.
- **الإنشاء للخادم وحده** (`create: if false`): السقف والاتجاه لا يُفحصان إلا بجمع الأرصدة من الدفتر كاملاً، وقد صار الفحص **داخل المعاملة** على قراءة جديدة للمجموعات الثلاث — المقارنة القديمة لـ`deposited` قبل/بعد لم تعد تكشف تسجيلاً مزدوجاً لأن السداد لا يمسّه.
- **حذفه ليّن ويُعيد الدين إلى القائمة** (`deletedAt` وحده، `affectedKeys().hasOnly(['deletedAt'])`)، فالتصحيح: احذف ← سجّل من جديد. السلة تستعيده للمسؤول.
- **يسافر مع كل ما يحمل الرصيد:** `readLedger` (فيدخل إغلاق الشهر وخروج المنتدَب وحذف الرحلة المسوّاة بلا تعديلها)، وكشف الحساب (سطرا «سدّد»/«استلم» بإشارتهما من `isCreditKind` وحدها)، والتقارير وExcel، والنسخة الاحتياطية والاستعادة.
- ⚠️ **ما يستنتج «المودَع» من الرصيد صار يطرح صافي السداد:** `depositedThisCycle` في كشف الحساب و`opening` في ملخص الدورة كانا يشتقّان من `remaining`، و`remaining` صار يشمل السداد — بلا الطرح كان ما استلمه المسافر يظهر نقصاً في «المودَع». والتسوية تختفي من القائمة لأن الرصيدَين صارا صحيحَين، لا لأن أحداً أشّر عليها — وهذا وحده ما يجعلها تبقى مختفية بعد إضافة مصاريف جديدة.

⚠️ **`mode` في `buildDepositLog` صار وسيطاً لهذا السبب تحديداً**: كان ثابتاً `'add'`، وسطرُ خصمٍ مكتوب بـ`'add'` يجعل `replayDepositLogs` (تدقيق «الرصيد = مجموع الحركات الموثّقة») يُضيف ما خُصم — فينكسر التدقيق **بصمت**، ويُظهر الدفتر سليماً وهو ليس كذلك.

**ولماذا دالة سحابية لا توسيع القواعد:** `travelers.update` و`depositLogs.create` كلاهما `isAdmin()`، فمنظّم الرحلة — وهو من يدير التحويلات فعلاً — لا يملك المسار من متصفحه (نفس سبب وجود `exitTraveler`). توسيع القاعدتين له كان يمنحه تعديل **أي** حقل في أي مستند مسافر (الاسم، الحذف الليّن) لا الرصيد وحده. الدالة تمنحه فعلاً واحداً بحدود يفحصها الخادم: الاتجاه (مَدين → دائن، ولا يُصحَّح مقلوباً بل يُرفض)، وسقف `min(الدين, الحقّ)` — تجاوزه يخلق ديناً لم يوجد — وإعادة قراءة الرصيدَين داخل المعاملة تمنع تسجيلاً مزدوجاً من ضغطتين متلاحقتين.

**والتأكيد خطوة ظاهرة** (نقرة تكشف «تأكيد» في السطر نفسه، بلا نافذة): حركة مالية لا تُتراجَع بضغطة لا يصحّ أن تمرّ بنقرة واحدة على زرٍّ في شبكة بطاقات. والتراجع الوحيد اليوم هو تحويل معاكس — تقييد مقصود: السطر المكتوب في سجلّ التدقيق لا يُحذف (`allow update, delete: if false`).

### 🆕 حدّ Firebase يُفرَض بقاعدة lint — لا بطبقة repository

**السؤال كان: هل نعزل Firebase خلف واجهات (`TripsRepository`، `ExpensesRepository`، `AuthProvider`) لنشتري خيار الهجرة بثمن رخيص؟ الجواب: لا — لأن الحدّ موجود أصلاً، ولأن الكلفة الحقيقية للهجرة ليست حيث تعمل الواجهات.** قِيس ذلك قبل القرار:

- **صفر مكوّنات** تستورد Firebase. الاستيراد محصور في `firebase.ts` و`firestore.ts` و21 خطّافاً (~3000 سطر). الخطّافات *هي* طبقة الـ repository فعلاً، بواجهة على مقاس React. التسرّب الوحيد كان `import type { User } from 'firebase/auth'` في المخزن — والمخزن لا يقرأ منه إلا `uid`.
- **كلفة الهجرة خارج أماكن الاستدعاء:** `firestore.rules` (655 سطراً، 27 دالة — الصلاحيات كلها تصير كوداً على الخادم)، و`functions/index.js` (~2000 سطر مع Admin SDK والـ custom claims)، ونقل البيانات والحسابات المربوطة. لا واجهة في العميل تلمس أياً منها.
- **الواجهة إما تتسرّب أو تُفقِر:** `onSnapshot` في 17 ملفاً، و`hasPendingWrites` (أساس `_pending`) في 4، و`persistentLocalCache` و`getDocsFromServer` لاستعادة المزامنة. واجهة تَعِد بـ «تحديث لحظي + عمل دون اتصال + كتابات معلّقة» مرسومة على مقاس Firestore فلا تنقل إلى غيره؛ وواجهة بأقل قاسم مشترك تُسقط هذه الميزات اليوم مقابل هجرة افتراضية.
- **`App.test.tsx` لا يحمي إعادة الهيكلة هذه:** يستبدل `./firebase` و`./hooks` كاملين، فيثبّت ما *فوق* الطبقة التي ستتغير. و13 ملف اختبار يستبدل `firebase/*` مباشرة كانت ستُعاد كتابتها لا أن تحمي.

**ما اتُّخذ بدلاً منه:**
1. `AppUser { uid: string }` في `types.ts` بدل `User` في `store/tripStore.ts` و`store/TripStoreProvider.tsx` والقصص. `User` من Firebase يطابقه بنيوياً، فيمرّ من `useAuth` دون تحويل.
2. قاعدة `no-restricted-imports` في `eslint.config.js` تمنع `firebase/*` و`../firebase` و`../firestore` في `src/` كلها **عدا** `src/hooks/` والملفين نفسيهما. **تحقّقنا من الحالة السالبة فعلاً:** ملفات تجريبية في `components/` و`store/` أنتجت 8 أخطاء، والملف نفسه في `hooks/` صفراً، و`@firebase/rules-unit-testing` لم يُلتقط.

⚠️ **`e2e/` واختبارات القواعد خارج نطاق القاعدة عمداً** — تختبر Firebase نفسه، فاستيراده هناك هو الغرض.

**3. النسخة الاحتياطية مستقلة عن Firestore — يُفحَص ذلك عند التصدير.** الصيغة كانت سليمة أصلاً: تواريخ `YYYY-MM-DD` نصية، وأوقات Unix ms رقمية، و`depositLogs` مسطّحة تحمل `travelerId`، وصفر `Timestamp` في الكود كله. لكن ذلك **عُرف لا ضمان**. `d.data() as Traveler` في `exportBackup` تحويل بلا تحقّق، ولو عُدِّل مستند من الكونسول أو بسكربت وصار فيه `Timestamp`، فإن `JSON.stringify` يكتبه `{seconds, nanoseconds}` بصمت، ويكتب `NaN` كـ `null`. التصدير ينجح والاستعادة ترفض الملف لاحقاً، أي بعد ضياع الأصل.

- `findNonPortableValue` في `utils/backup.ts` يقبل null ونصاً ومنطقياً ورقماً منتهياً ومصفوفة وكائناً عادياً فقط. `buildTripBackup` يرمي `BackupNotPortableError` بمسار أول قيمة مخالفة (`$.travelers[2].deletedAt`)، و`exportBackup` يعرضه للمنظّم بدل النص العام.
- **لا يستورد Firebase ليميّز `Timestamp`:** كل كائنات الـ SDK نسخ من أصناف، ونموذج البيانات لا يحوي صنفاً واحداً، فرفض أي نموذج أصل غير `Object.prototype` يكفي.
- `undefined` مقبول كقيمة خاصية (يسقط المفتاح، وهذا معنى الحقل الاختياري الغائب)، ومرفوض داخل مصفوفة لأنه يصير `null` هناك.
- **اختبار يثبت الاستقلال لا الشكل وحده:** الملف بعد `JSON.parse(JSON.stringify(...))` يعيد إنتاج `calculateBalances` و`calculateSettlements` كما هي، بلا قاعدة بيانات.
- **الحالة السالبة جُرّبت فعلاً:** تعطيل الرمي أسقط 6 اختبارات.
- **فُحصت بيانات الإنتاج قراءةً فقط** (2026-09-12، بنفس المنطق عبر Admin SDK): رحلتان و255 مستنداً، صفر قيم مخالفة. الحارس لا يمنع أي تصدير اليوم.

⚠️ **`travelerNames` تفصيل Firestore داخل الملف ولم يُحذف عمداً.** هو حجز التفرّد الذي تفرضه القواعد، ويمكن اشتقاقه من المسافرين النشطين. لكنه JSON عادي لا يربط الملف بأي SDK، وحذفه يستلزم رفع `schemaVersion` وتعديل `restoreTrip` ونشر الدوال، مقابل لا شيء. النظام البديل يتجاهله إن شاء.

**متى يُعاد فتح القرار:** حين يظهر سبب هجرة *فعلي* (فاتورة تتجاوز حداً، أو حاجة لاستعلامات علائقية لا يقدر عليها Firestore). حينها ابدأ من القواعد والدوال والبيانات، لا من واجهات العميل. وتأمين الخروج الأرخص هو البيانات — البند ٣ أعلاه يجعله مفحوصاً لا مفترضاً.
...
| الاقتراح | النتيجة المقيسة |
|---|---|
| `isolate: false` | **26 اختباراً ساقطاً في 9 ملفات** — حالة عامة تتسرّب بين الملفات، فيصير الترتيب جزءاً من النتيجة |
| `pool: 'vmThreads'` | **12 ساقطاً في `useInviteJoin.test.ts`** — «Cannot redefine property: location»؛ `window.location` غير قابل لإعادة التعريف داخل عالم VM، والملف يعيد تعريفه في `beforeEach` |

المهم في الأول ليس عدد الساقطين بل نوعه: إطفاء العزلة لا «يكشف» تسرّباً موجوداً فحسب، بل يجعل نجاح المجموعة دالةً في ترتيب تشغيل الملفات. مجموعةٌ تحرس حسابات مالية لا تُقايَض عزلتها بثمانِ ثوانٍ.

**المتّبع بدلاً منهما:** الملفات التي لا تلمس DOM إطلاقاً تعلن `// @vitest-environment node` في سطرها الأول — 18 ملفاً، والعزلة كاملة كما كانت، والزمن من ~11.2s إلى ~8.7s (−21%) بصفر اختبار ساقط.

**والحدّ ليس «هل يمرّ الملف تحت node؟» بل «هل يظلّ يفحص الشيء نفسه؟».** `src/utils/tripId.test.ts` يمرّ تحت node — ومع ذلك استُثني. السبب أن `readExplicitTripId()` يلفّ قراءة `window.location.search` بـ`try/catch` يبتلع غياب النافذة ويرجع للرحلة الافتراضية، فالاختبار تحت node يؤكّد الافتراضي عبر مسار الاستثناء لا عبر التحليل الفعلي. وتُحقِّق ذلك بالكسر لا بالاستشهاد: بزرع `return 'sabotage-trip'` داخل فرع الـ`try`، سقط الملف تحت jsdom (2/11) و**مرّ كاملاً تحت node (11/11)** — أي أن النقل كان سيحوّله إلى أخضر كاذب. استُثني `preload.test.ts` كذلك لأنه يحتاج `requestIdleCallback`، لكنه يسقط بصوت عالٍ لا بصمت.

**ولهذا تبقى jsdom هي الافتراضي:** الملف الجديد يرث البيئة الآمنة، ومن يريد `node` يطلبها صراحةً في ملفه. الإعلان في أعلى الملف لا في نمط داخل الإعداد، حتى لا يُسحب ملفٌ جديد إلى `node` بصمت لمجرّد وقوع مساره تحت نمط.

**و`src/setupTests.ts` صار يستورد مطابِقات jest-dom بشرط `typeof window !== 'undefined'`** — وفرٌ صغير (~0.3s) لكنه صحيح مبدئياً. وتُحقِّق أن الشرط في الجهة الصحيحة بقلبه: انقلبت اختبارات المكوّنات إلى «Invalid Chai property: toBeInTheDocument» فوراً. ⚠️ و`export {}` في ذلك الملف ليست زخرفة: بدونها يعدّه TypeScript سكربتاً لا وحدة فيرفض الـ`await` في المستوى الأعلى (TS1375) — **والاختبارات تمرّ خضراء رغم ذلك** لأن Vite يحوّل بلا فحص أنواع؛ كشفه `npm run typecheck` وحده.
...
### 🆕 قاموس الواجهة: كلمة واحدة لكل شيء، وسبعة مرادفات حُذفت

طلب صاحب الحساب قاموساً صغيراً ثابتاً. المشكلة لم تكن كلمات «سيئة» بل **كلمتين لشيء واحد** في شاشتين متجاورتين — وهو ما يجعل المستخدم يظنّ أنهما شيئان.
...
**⚠️ و«عضو» لم تكن مرادفاً بريئاً بل تسريباً للنموذج الداخلي.** `members` (حسابات تحمل claim) و`travelers` (ملفات في الدفتر) **مجموعتان مختلفتان فعلاً**، ويمكن أن يوجد مسافر بلا حساب («مسجل يدوياً») أو حساب بلا مسافر. لكن الواجهة **تعرضهما أصلاً كقائمة واحدة** والفرق فيها **شارة** (`منضم` / `مسجل يدوياً`) لا اسم ثانٍ. فالكلمة الثانية كانت تكراراً لما تقوله الشارة — والتمييز الحقيقي لم يُفقد بحذفها.

⚠️ وحيث بقيت الحاجة لتسمية حسابٍ لم يُربط بعد، استُعملت **«منضمّ»** — وهي نصّ الشارة نفسها، لا مصطلح ثامن.

**⚠️ و«المسؤول» حُذف من الواجهة بلا أن يُحذف الدور.** `isAdmin` (عالمي) و`isOrganizer` (لهذه الرحلة) صلاحيتان مختلفتان في `firestore.rules` ولم تتغيّرا بحرف. ما تغيّر أن الواجهة تتكلّم **بدورٍ واحد**:
- رسالة يراها من لا صلاحية له إطلاقاً → «هذا الإجراء متاح لمنظّم الرحلة فقط.» — وهي **صادقة لكل من يراها**، لأن من يملك الصلاحية العامة لا يُمنع أصلاً فلا يقرؤها.
- رسالة يراها منظّمٌ مُنع من فعلٍ عام → **تصف الحدّ لا تسمّي دوراً ثانياً**: «هذا الإجراء ليس من صلاحيات منظّم الرحلة.»

**⚠️ وكشفت المراجعة كذبةً في شريط الترحيب، لا مجرّد مصطلح:** «**وضع المسؤول** (الزر أعلى الصفحة) مخصص لمنظّم الرحلة فقط» — يخلط مصطلحين لدورٍ واحد في جملة واحدة، **ويشير إلى زرّ لم يعد موجوداً** (أُزيل مع تدفّق المصادقة الثاني). أول ما يقرؤه كل مستخدم جديد كان إرشاداً لزرّ غير موجود.

**⚠️ وبقيّة من قرار سابق:** تغيير 2026-09-09 حذف «دورة»/«فترة» من الواجهة وسمّى جدول الأشهر «الأشهر» — لكن **نسخة الطباعة من نفس الجدول** بقيت «ملخص الفترة». الجدول نفسه باسمين حسب الشاشة أو الورق. وشارة الشهر الحالي بقيت «الحالية» (مطابِقةً لـ«الدورة» المحذوفة) بينما العمود «الشهر» — صارت «الحالي».

**⚠️ و«المتبقي» حُذفت لصالح «الرصيد» — والسبب أن الأولى تكذب عند الإشارة السالبة.** `remaining` رقمٌ **مُوقَّع**: سالبه يعني أن نصيب المسافر تجاوز ما أودعه، أي أنه مَدين. و«المتبقي» تعني «ما بقي» — فبطاقة تقول «المتبقي ‎-150.00» تناقض نفسها في الحالة التي تهمّ أكثر من غيرها: من يدين لمن. أما «الرصيد» فمُوقَّع بطبيعته، ورصيدٌ سالب دَينٌ بلا التواء.

وشاهدٌ ثانٍ: قسم الشاشة الرئيسية اسمه **«أرصدة المسافرين»** منذ البداية، فالحقل داخله كان يخالف عنوانه. وExcel كان يجمع الكلمتين في خلية واحدة حرفياً: `['الرصيد المتبقي', …]`.

⚠️ والتغيير يمسّ **أرقاماً مطبوعة ومصدَّرة** (ترويسة عمود في PrintDocs وExcel) تُقرأ خارج التطبيق — وهذا سبب عرضه على صاحب الحساب أولاً لا تنفيذه بوصفه تنظيفاً.

**⚠️ وإملاء «المودَع» بالفتحة ليس زينة.** بدونها تُقرأ «المُودِع» — أي *الشخص* الذي أودع لا *المبلغ* المودَع، وهو عكس المقصود تماماً. كان الإملاءان يتناوبان في التطبيق (البطاقة بلا فتحة، التقارير بها)؛ اعتُمدت الفتحة في الجميع.
...
**What remained was a pattern, not a count: a window that closes itself to open another.** Of the nine states, seven are destinations the user opens on purpose. The other two were hand-offs:
- `monthlyRollover` — «إغلاق الشهر» inside «هذا الشهر» closed that window to open the confirmation in its place. Nothing else ever opened it.
- `exitTraveler` — the traveler profile (local state in `TravelerCard`, *outside* the union) closed itself to open the exit window (*inside* the union): a hand-off across the boundary of the state machine meant to prevent two windows at once.

**Both became steps inside their own window.** `RolloverConfirm` replaces `LongTermPanel`'s content in `LongTermModal`; cancel returns to it, success still closes via `closeModal` in `useTripWorkspace`. `ExitTravelerConfirm` replaces the exit button inside `TravelerProfileModal` — the same move «تعديل الرصيد» made earlier (`DepositEditor`). The confirmation *replaces* its trigger rather than appearing below it: «تسوية وخروج من الرحلة» and «تسوية وخروج» side by side would be confusing, and a substring match in the E2E spec would hit both.

**Discoverability was the constraint.** The owner once asked "where is removing a traveler who wants to leave?" — so the long-term trash button on the card still reaches the exit in one tap: it opens the profile with the confirmation already showing (`initiallyOpen`). `TravelerSection.test.tsx` guards that path, and that re-opening the card normally does not start on the confirmation.

**What fell away with it:** `requestDeleteTraveler`'s standard/long-term branch (the card branches instead; the `describeExitBlockFor` guard in `useTravelerActions` stays as the safety net), `ModalManager`'s whole `longTerm` prop, two lazy importers, and the `openRollover` plumbing through `useTripWorkspace` → `App.tsx`. `ModalState` is seven states, all destinations.

**Two things this surfaced:** the E2E exit tests located the confirmation by `role="dialog"`, which matched the old standalone window, not the flow, so they now locate it within the profile. And the profile's close button was icon-only with no accessible name, so it now has `aria-label="إغلاق ملف المسافر"`.

### 🆕 The store lost its `form` slice — a field earns global state by having *repeated* readers, not by being volatile

**Asked by an outside reviewer: does every store field actually need to be global?** Measured field by field rather than argued: exactly three components read the store — `ExpenseListItem` (one per row inside Virtuoso), `TravelerCard` (one per traveler), and `ExpenseForm`. The first two are why the store exists: repeated, `memo()`-wrapped components that cannot take props through a virtualized list and must not re-render per keystroke. **`ExpenseForm` is neither** — a single instance that `App.tsx` renders directly, with no props at all. So its state travelled `App` → `TripStoreProvider` props → `useLayoutEffect` → store → selector → `ExpenseForm`: a round trip through global state to deliver a value its own parent already held, costing a second synchronous render pass per keystroke.

**Removed: the whole `form` slice (7 fields), plus `actions.cancelExpenseForm` and `data.ratesUpdatedAt`** — every one of them had `ExpenseForm` as its only reader. `ExpenseForm` now takes all thirteen as props from `App.tsx` (`ExpenseFormProps` in `ExpenseSection.tsx`) and no longer reads the store. The store keeps two slices and twelve fields, each with a repeated reader. This is the same test the context split already applied once — six fields were dropped then because "no component ever read them through context — they are passed as props from `App.tsx`" (above).

**The volatility rule (guideline 16) is not weakened — it is now structurally satisfied:** nothing that changes per keystroke is in global state at all, so there is no volatile slice left to leak into. `memo()` stays on `ExpenseForm`, because `App` re-renders for reasons unrelated to the form (toasts, sync status, snapshots) and every prop is identity-stable unless it actually changed.

**Not done, deliberately:** the form *state* did not move into the component. `newExpense` is written from three places outside the form — `startEditExpense` (a list row), `openExpenseForm` (the input bar and the empty state) — and read by `hasUnsavedData` before an app update. `useExpenseActions` is its right owner; only the delivery route was wrong.

**A gap this exposed:** with the wiring moved into `App.tsx`, passing the form an empty traveler list survived every existing test — tried on purpose before writing the guard. `App.test.tsx` now asserts the open form receives the trip's travelers, and that test fails under the same mutation.
...
🆕 **The `online` trigger is gone; only `visibilitychange` remains.** An outside review asked the right question about this whole area — *does the SDK already handle this case?* — and for `online` the answer was in the SDK's own source, not in memory: `BrowserConnectivityMonitor` listens for `online` and calls `restartNetwork()`, which tears down and re-creates the streams (`@firebase/firestore` 4.17.2). The entry above had *already measured* that path at 93 ms and said so, yet the hook listened for `online` anyway — a full server read fired at the exact moment the SDK was reconnecting on its own, carrying the pending-writes hazard for nothing. It also explains itself in the sentence above: the in-flight lock was justified partly by "`visibilitychange` and `online` routinely fire together", i.e. a guard against a trigger that should not have existed. The `visibilityState === 'visible'` check stays, for a different reason that survives: `visibilitychange` fires on hide too.

🆕 **Field-verified on a real phone, 2026-09-20 — the first time this scenario was tested outside an emulator.** The owner opened the app on their phone, switched away to WhatsApp and TikTok (heavy enough that the OS freezes a backgrounded PWA tab), left it there for a while, edited an expense from a desktop browser on the same account, and returned to the phone: the change was already there, with no pull-to-refresh. **What that proves is the user-visible guarantee — no stale data on resume — not which mechanism delivered it**: a recovery read and a stream the SDK revived on its own look identical from the outside, and nothing in the UI distinguishes them. That is exactly why the mechanism is not being swapped out (below): the observable behaviour is correct today, measured on the device that matters. (An earlier attempt at this test — both devices open and visible at once — proved only that live `onSnapshot` works; the hook never runs while the app stays in the foreground.)

**Why `visibilitychange` is not removed with it.** The SDK listens for that event as well, but for two other purposes, both read from its source: the foreground tab asks for the primary lease (`updateClientMetadataAndTryBecomePrimary`), and the async queue retries backed-off IndexedDB work. Neither restarts the network nor checks that the existing stream is still alive — which is exactly the frozen-PWA case this hook exists for. **The remaining candidate is deeper and was deliberately not taken:** replacing the `getDocsFromServer` read with the SDK's own `disableNetwork`/`enableNetwork` restart would deliver the fresh data through `onSnapshot` (which preserves pending writes), retiring both the pending-writes guard and the narrow race it admits it cannot cover. It is not done because it cannot be verified here: freezing a PWA tab is not reproducible in Vitest or Playwright, and this is the one area where an unverified "simplification" reintroduces silent staleness. It waits for a field test on a real phone.
