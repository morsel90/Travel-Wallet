...
| Cloud Functions | `firebase deploy --only functions` | `manageTrip`, `manageInvite`, `joinViaInvite`, `updateMyTravelerName`, `linkTravelerAccount`, `manageMember`, `restoreTrip`, 🆕 `recordSettlement`, 🆕 `recordDeposit` |
...
🆕 **`vercel.json` holds exactly one rewrite, and it is not a function proxy** — `/__/auth/:path*` → `https://travelapp-87206.firebaseapp.com/__/auth/:path*`. It exists so Google sign-in can run on the app's **own** origin: browsers now partition third-party storage, so a popup that completes on `travelapp-87206.firebaseapp.com` can lose the session on the way back to `travel-app-final-nu.vercel.app` — the user picks an account and lands on the sign-in screen again. Setting `VITE_FIREBASE_AUTH_DOMAIN` to the app's own host makes the SDK use this path instead, and the handler is same-origin from then on. Two things make it safe: the service worker already refuses to hijack it (`navigateFallbackDenylist: [/^\/__/]` in `vite.config.js`), and the app's host is already in Firebase's authorized-domains list.

⚠️ **Three switches, not one — and the order matters.** Flipping `VITE_FIREBASE_AUTH_DOMAIN` alone breaks Google sign-in for everyone with `Error 400: redirect_uri_mismatch` (this happened on 2026-09-19 and was rolled back within minutes). Google only returns the result to addresses on the OAuth client's own list, which lives in Google Cloud, not Firebase:

1. **The rewrite is live** — `https://<app host>/__/auth/handler` returns 200, and its body matches `https://<project>.firebaseapp.com/__/auth/handler`. A 200 alone proves nothing: an SPA returns its own `index.html` for any path.
2. **Google Cloud › APIs & Services › Credentials › "Web client (auto created by Google Service)"** — add `https://<app host>` to *Authorized JavaScript origins* and `https://<app host>/__/auth/handler` to *Authorized redirect URIs*. Keep the existing `firebaseapp.com` entries: Preview still uses them. The console must be open as the **project owner's** account; other accounts see "You need additional access".
3. **Only then** set `VITE_FIREBASE_AUTH_DOMAIN` (Production) and redeploy.

Checking step 2 needs no sign-in: open `https://accounts.google.com/o/oauth2/v2/auth?client_id=<id>&redirect_uri=<url-encoded handler>&response_type=code&scope=openid`. An accepted address lands on the sign-in page; a rejected one lands on `/signin/oauth/error`. Run it with a made-up address as a control, so you know the probe can tell the two apart.

⚠️ **The destination is a literal project URL — the very trap the `/api/*` rewrites fell into.** It is safe *only* because nothing uses it unless `VITE_FIREBASE_AUTH_DOMAIN` names the app's own host, which is set for **Production only**. A preview/staging build keeps its own project's `*.firebaseapp.com` authDomain and never touches this path. If a second environment ever needs same-origin auth, it needs its own `vercel.json` destination — Vercel does not interpolate env vars in rewrites, which is exactly why the function proxy was deleted. Functions are reached through `httpsCallable`, which derives the URL from `projectId` and handles CORS itself. **Adding a Cloud Function therefore needs no frontend deployment config at all**; deploy the function and call it. (Historically both were proxied through `/api/*` rewrites pointing at literal function URLs, which silently pinned every build to one Firebase project — see *Environment Variables*.)
