...
| 🆕 «الدخول عبر Google لا يعمل من هذا العنوان» (كان سابقاً: «جرّب متصفحاً آخر») | The page was opened from a **per-deployment Vercel URL** (`travel-app-final-<hash>-mohammed-alathim-s-projects.vercel.app`), typically from Vercel's deploy email. Firebase Auth's authorized domains take no wildcards, so Google sign-in (popup *and* redirect) fails there with `auth/unauthorized-domain`. The browser is irrelevant | Open `https://travel-app-final-nu.vercel.app` — the login screen now shows that link itself (`utils/canonicalUrl.ts`). Email sign-in works from any address. **Do not** add the deployment URL to Firebase's list: it changes every deploy |
...
| 🆕 …or with "firebase-tools no longer supports Java version before 21" | A JDK *is* installed but older than 21; `firebase-tools` 15 raised its floor | Install JDK 21+ (`brew install openjdk@21`) and make it the active `java`. ⚠️ This one bites CI, not just laptops: the `ubuntu-latest` preinstalled JDK was older, so the `rules` job failed before running a single test until `actions/setup-java` was pinned |
