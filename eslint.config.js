...
export default tseslint.config(
...

  {
    ...
    rules: {
      ...
      //
      // قيس أثره على هذا المستودع قبل استبعاده — 26 مخالفة في 21 ملفاً،
      // وليست فيها مخالفة واحدة تمثّل خطأً فعلياً:
      //   • 14 × set-state-in-effect — كل خطّاف يشترك في onSnapshot
      //     (useExpenses، useTravelers، useTripMembers…). القاعدة تريد
      //     useSyncExternalStore بدلاً منه؛ أي إعادة كتابة 14 خطّاف بيانات.
      //   • 7 × refs — قراءة storeRef.current أثناء الرسم في
      //     TripStoreProvider، وهي عين الغرض من نمط المتجر الثابت.
      //   • immutability — إسناد window.location.href (تنقّل مقصود وموثَّق)،
      //     ومرجع ذاتي داخل مغلّف لا يُنفَّذ إلا بعد التصريح.
      //   • purity — Date.now() كقيمة ابتدائية لـ useRef.
      //
