...
/**
 * 🆕 أرقام الشهر الجاري — undefined/null في الرحلة القياسية، فلا يظهر أي ذكر
 * للشهر في السطر الموجز بحرف (نفس مبدأ `longTerm` في useAppCoordinator: قيمة
 * واحدة تُفحص هنا بدل شرط `tripType` داخل هذا الملف).
 *
 * ⚠️ **بلا `periodLabel` عمداً.** كان السطر يقول «دورة أغسطس 2026 · المتبقي …»
 * فيحمل مفهومين لا يعني المستخدمَ أيٌّ منهما على الشاشة الرئيسية: كلمة «دورة»
 * (اصطلاح داخلي بحت) واسم شهرٍ هو الشهر الجاري بداهةً. الرئيسية تقول «هذا
 * الشهر» وكفى؛ الاسم الصريح يبقى حيث يلزم فعلاً — التقارير والطباعة وExcel،
 * حيث تُقرأ الأرقام بعد شهور وخارج التطبيق.
 */
export type HeaderCycleStats = HeaderStats
...
const Header = ({
  ...
  // الأصلية لا حلّها. لا تبديل دورة/تراكمي بعد الآن — الشهر الجاري فقط حين
  // يتوفّر (cycleStats)، وإلا فالإجمالي كما كان قبل ميزة الأشهر أصلاً.
  //
  // 🆕 الفرق بين الحالتين كلمتان لا مفهوم: «المتبقي» في الرحلة القياسية،
  // «المتبقي هذا الشهر» في الطويلة. الرقم يختلف مصدره، أما ما يقرؤه المستخدم
